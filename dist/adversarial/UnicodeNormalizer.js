...
const INVISIBLE_REGEX = new RegExp('[' + Array.from(INVISIBLE_CHARS).map(cp => '\\u' + cp.toString(16).padStart(4, '0')).join('') + ']', 'g');
// Build homoglyph regex for detection
const HOMOGLYPH_REGEX = new RegExp('[' + Array.from(HOMOGLYPH_MAP.keys()).map(char => {
...
class UnicodeNormalizer {
    ...
    static normalize(text) {
        ...
        const suspiciousScore = this.calculateSuspicion(text, invisibleCharCount, homoglyphCount);
        return {
            normalized,
            original: text,
     ...
     */
    static calculateSuspicion(text, invisibleCount, homoglyphCount) {
        if (text.length === 0)
            return 0;
        let score = 0;
