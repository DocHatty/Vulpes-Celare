...
exports.InterPHIDisambiguator = exports.EnsembleVoter = void 0;
...
class EnsembleVoter {
     ...
     */
    normalizedEntropy(positiveCount, totalCount) {
        if (totalCount <= 1)
            return 0; // Single signal = no disagreement possible
        // Use cached entropy calculation
        const cacheKey = ComputationCache_1.ComputationCache.entropyKey(positiveCount, totalCount);
        return this.cache.getEntropy(cacheKey, () => {
            ...
            if (pPositive < EnsembleVoter.EPSILON ||
                pNegative < EnsembleVoter.EPSILON) {
                return 0; // Full agreement
            }
            // Binary entropy: H = -p*log2(p) - (1-p)*log2(1-p)
            const entropy = -(pPositive * Math.log(pPositive) + pNegative * Math.log(pNegative)) /
                EnsembleVoter.LOG2;
    ...
    weightedGeometricMean(signals) {
        ...
        if (totalWeight < EnsembleVoter.EPSILON)
            return 0;
     ...
     * Reference: Good (1950)
     */
    bayesianLogOddsCombination(signals) {
     ...
     */
    calculateAgreementMultiplier(positiveSignals, totalSignals) {
        if (totalSignals < 2)
            return 1.0; // No adjustment for single signal
     ...
     */
    static patternSignal(confidence, patternName) {
        return {
            source: "PATTERN",
            weight: 1.0,
            confidence,
     ...
     */
    static dictionarySignal(confidence, dictionaryType, fuzzyMatch = false) {
        return {
            source: "DICTIONARY",
     ...
     */
    static contextSignal(confidence, contextType) {
        return {
            source: "CONTEXT",
            weight: 1.0,
            confidence,
     ...
     */
    static structureSignal(confidence, position) {
        return {
            source: "STRUCTURE",
            weight: 1.0,
            confidence,
     ...
     */
    static labelSignal(confidence, labelText) {
        return {
            source: "LABEL",
            weight: 1.0,
            confidence,
     ...
     */
    static chaosSignal(confidence, chaosLevel) {
        return {
    ...
    quickVote(options) {
        ...
        if (options.patternMatch) {
            signals.push(EnsembleVoter.patternSignal(options.patternMatch.confidence, options.patternMatch.name));
        }
        if (options.dictionaryMatch) {
            signals.push(EnsembleVoter.dictionarySignal(options.dictionaryMatch.confidence, options.dictionaryMatch.type, options.dictionaryMatch.fuzzy));
        }
        if (options.contextMatch) {
            signals.push(EnsembleVoter.contextSignal(options.contextMatch.confidence, options.contextMatch.type));
        }
        if (options.labelMatch) {
            signals.push(EnsembleVoter.labelSignal(options.labelMatch.confidence, options.labelMatch.label));
        }
        if (options.structureMatch) {
            signals.push(EnsembleVoter.structureSignal(options.structureMatch.confidence, options.structureMatch.position));
        }
        if (options.chaosAdjustment) {
            signals.push(EnsembleVoter.chaosSignal(options.chaosAdjustment.confidence, options.chaosAdjustment.level));
        }
        return this.vote(signals);
    }
    generateExplanation(signals, score, recommendation) {
...
class InterPHIDisambiguator {
     ...
     */
    static countMatches(patterns, context) {
        let count = 0;
    ...
    static disambiguateDate(text, context, alternativeType) {
        ...
        if (alternativeType === "AGE") {
            const ageMatches = InterPHIDisambiguator.countMatches(InterPHIDisambiguator.AGE_INDICATORS, lowerContext);
            altScore = ageMatches * 0.4;
        }
        if (alternativeType === "MEASUREMENT") {
            const measurementMatches = InterPHIDisambiguator.countMatches(InterPHIDisambiguator.MEASUREMENT_INDICATORS, lowerContext);
    ...
    static disambiguateName(text, context, alternativeType) {
        ...
        if (alternativeType === "MEDICATION") {
            const medMatches = InterPHIDisambiguator.countMatches(InterPHIDisambiguator.MEDICATION_INDICATORS, lowerContext);
            altScore = medMatches * 0.35;
        }
        if (alternativeType === "DIAGNOSIS") {
            const dxMatches = InterPHIDisambiguator.countMatches(InterPHIDisambiguator.DIAGNOSIS_INDICATORS, lowerContext);
