...
exports.contextualConfidenceModifier = exports.ContextualConfidenceModifier = void 0;
exports.isContextModifierEnabled = isContextModifierEnabled;
 ...
 */
class ContextualConfidenceModifier {
    config;
    constructor(config = {}) {
    ...
    analyzeDocumentContext(text) {
        ...
        for (const pos of samplePositions) {
            const context = ClinicalContextDetector_1.ClinicalContextDetector.analyzeContext(text, pos, 100);
        ...
        if (moderateCount >= 2)
            return "MODERATE";
        if (moderateCount >= 1 || weakCount >= 2)
            return "WEAK";
        return "NONE";
    }
    /**
     ...
     */
    modifySpanWithDocContext(span, docContext) {
        ...
        if (this.config.contextIndependentTypes.has(span.filterType)) {
            if (docContext === "STRONG" || docContext === "MODERATE") {
                const boost = docContext === "STRONG"
                    ? this.config.strongContextBoost * 0.3
                    : this.config.moderateContextBoost * 0.3;
                span.confidence = Math.min(this.config.maximumConfidence, originalConfidence + boost);
                return {
                    span,
                    originalConfidence,
                    modifiedConfidence: span.confidence,
        ...
        if (!isBorderline) {
            ...
            switch (docContext) {
                case "STRONG":
                    span.confidence = Math.min(this.config.maximumConfidence, originalConfidence + this.config.strongContextBoost * 0.5);
                    modification = "BOOSTED";
                    reason = "Borderline detection in STRONG clinical document";
                    break;
                case "MODERATE":
                    span.confidence = Math.min(this.config.maximumConfidence, originalConfidence + this.config.moderateContextBoost * 0.5);
                    modification = "BOOSTED";
                    reason = "Borderline detection in MODERATE clinical document";
                    break;
                case "WEAK":
                    span.confidence = Math.min(this.config.maximumConfidence, originalConfidence + this.config.weakContextBoost * 0.5);
                    modification = "BOOSTED";
    ...
    modifySpan(span, text) {
        ...
        const context = ClinicalContextDetector_1.ClinicalContextDetector.analyzeContext(text, span.characterStart, span.characterEnd - span.characterStart);
        ...
        if (this.config.contextIndependentTypes.has(span.filterType)) {
            ...
            if (context.strength === "STRONG" || context.strength === "MODERATE") {
                const boost = context.strength === "STRONG"
        ...
        switch (context.strength) {
            case "STRONG":
                span.confidence = Math.min(this.config.maximumConfidence, originalConfidence + this.config.strongContextBoost);
                modification = "BOOSTED";
                reason = `Strong clinical context (${context.indicators.length} indicators)`;
                break;
            case "MODERATE":
                span.confidence = Math.min(this.config.maximumConfidence, originalConfidence + this.config.moderateContextBoost);
                modification = "BOOSTED";
                reason = `Moderate clinical context (${context.indicators.length} indicators)`;
                break;
            case "WEAK":
                span.confidence = Math.min(this.config.maximumConfidence, originalConfidence + this.config.weakContextBoost);
                modification = "BOOSTED";
    ...
    static summarize(results) {
        ...
        const avgBoost = boosted.length > 0
            ? boosted.reduce((sum, r) => sum + (r.modifiedConfidence - r.originalConfidence), 0) / boosted.length
            : 0;
        const avgPenalty = penalized.length > 0
            ? penalized.reduce((sum, r) => sum + (r.originalConfidence - r.modifiedConfidence), 0) / penalized.length
            : 0;
        const byContextStrength = {
            STRONG: 0,
            MODERATE: 0,
            WEAK: 0,
            NONE: 0,
...
exports.ContextualConfidenceModifier = ContextualConfidenceModifier;
/**
 * Singleton instance with default configuration
 */
exports.contextualConfidenceModifier = new ContextualConfidenceModifier();
/**
 ...
 */
function isContextModifierEnabled() {
    return process.env.VULPES_CONTEXT_MODIFIER === "1";
}
//# sourceMappingURL=ContextualConfidenceModifier.js.map