...
class MLWeightOptimizer {
     ...
     */
    generateSyntheticTrainingData(documents) {
     ...
     */
    evaluateWeights(weights, votingConfig, documents) {
        const scorer = new WeightedPHIScorer_1.WeightedPHIScorer(weights, votingConfig.redactThreshold || 0.50);
        let truePositives = 0;
    ...
    optimizeGridSearch() {
        ...
        const generateGridPoints = (min, max) => {
            const points = [];
        ...
        // Threshold grid
        const thresholdGrid = generateGridPoints(MLWeightOptimizer.VOTING_BOUNDS.redactThreshold[0], MLWeightOptimizer.VOTING_BOUNDS.redactThreshold[1]);
    ...
    optimizeGenetic() {
        ...
        for (const individual of population) {
            const metrics = this.evaluateWeights(individual.weights, individual.votingConfig, this.trainingData);
        ...
        for (let gen = 0; gen < generations; gen++) {
            ...
            while (nextGeneration.length < populationSize) {
                ...
                // Evaluate offspring
                const metrics = this.evaluateWeights(offspring.weights, offspring.votingConfig, this.trainingData);
    ...
    createRandomIndividual() {
        ...
        for (const [key, bounds] of Object.entries(MLWeightOptimizer.WEIGHT_BOUNDS)) {
            ...
            weights[key] = min + Math.random() * (max - min);
        }
        const votingConfig = {
    ...
    crossover(parent1, parent2) {
        ...
        for (const key of Object.keys(MLWeightOptimizer.WEIGHT_BOUNDS)) {
            if (Math.random() < 0.5) {
                weights[key] = parent1.weights[key];
            }
            else {
                weights[key] = parent2.weights[key];
            }
        }
    ...
    mutate(individual) {
        ...
        const bounds = MLWeightOptimizer.WEIGHT_BOUNDS[keyToMutate];
        // Gaussian mutation
        const currentValue = individual.weights[keyToMutate] || (bounds[0] + bounds[1]) / 2;
        ...
        individual.weights[keyToMutate] = newValue;
        ...
        if (Math.random() < 0.3) {
            ...
            const configBounds = MLWeightOptimizer.VOTING_BOUNDS[configKey];
            const currentConfigValue = individual.votingConfig[configKey] ||
            ...
            individual.votingConfig[configKey] = newConfigValue;
        }
    }
    /**
     ...
     */
    optimizeRandomSearch(iterations = 1000) {
        if (this.trainingData.length === 0) {
            throw new Error('No training data provided. Call addTrainingData() first.');
        }
        let bestF1 = -1;
