...
class WindowService {
     ...
     */
    static getWindow(text, span, options = {}) {
        ...
        const windowEnd = Math.min(tokensWithPos.length, spanTokenEnd + 1 + windowSize);
     ...
     */
    static getWindowAt(text, start, end, options = {}) {
        const windowSize = options.size ?? this.DEFAULT_WINDOW_SIZE;
        const includePunctuation = options.includePunctuation ?? false;
        const toLowerCase = options.toLowerCase ?? false;
        // Tokenize with positions to avoid indexOf bugs
     ...
     */
    static tokenize(text, includePunctuation) {
     ...
     */
    static containsKeyword(window, keywords) {
     ...
     */
    static matchesPattern(window, patterns) {
     ...
     */
    static getTokensBefore(text, span, count = 5) {
     ...
     */
    static getTokensAfter(text, span, count = 5) {
        const tokensWithPos = this.tokenizeWithPositions(text, false);
        let spanTokenEnd = -1;
        for (let i = 0; i < tokensWithPos.length; i++) {
            const token = tokensWithPos[i];
     ...
     * @param options - Window extraction options
     */
    static populateWindows(text, spans, options = {}) {
