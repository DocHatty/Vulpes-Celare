...
exports.ReplacementContextService = exports.ReplacementScope = void 0;
var ReplacementScope;
(function (ReplacementScope) {
    /** Same value gets same replacement across entire document */
    ReplacementScope["DOCUMENT"] = "DOCUMENT";
    /** Same value gets same replacement within same context */
    ReplacementScope["CONTEXT"] = "CONTEXT";
    /** No consistency - each occurrence gets unique replacement */
    ReplacementScope["NONE"] = "NONE";
})(ReplacementScope || (exports.ReplacementScope = ReplacementScope = {}));
/**
 * Replacement Context Service
...
class ReplacementContextService {
     ...
     */
    getReplacement(originalValue, filterType, context, generator) {
     ...
     */
    hasSeen(originalValue, filterType, context) {
        if (this.scope === ReplacementScope.NONE) {
            return false;
        }
        const key = this.makeKey(originalValue, filterType, context);
     ...
     */
    getExistingReplacement(originalValue, filterType, context) {
        if (this.scope === ReplacementScope.NONE) {
            return null;
        }
        const key = this.makeKey(originalValue, filterType, context);
     ...
     */
    getReplacementsForType(filterType) {
        const map = this.getMap();
        const entries = [];
     ...
     * @returns Array of replacement entries
     */
    getReplacementsForContext(context) {
        const map = this.getMap();
        const entries = [];
        for (const entry of map.values()) {
    ...
    getStatistics() {
        return {
            ...
            consistencyRate: this.stats.totalReplacements > 0
    ...
    import(data) {
        ...
        if (data.entries) {
            ...
            for (const entry of data.entries) {
                const key = this.makeKey(entry.originalValue, entry.filterType, entry.context);
                map.set(key, entry);
            }
        }
    }
     ...
     */
    makeKey(originalValue, filterType, context) {
...
exports.ReplacementContextService = ReplacementContextService;
//# sourceMappingURL=ReplacementContextService.js.map