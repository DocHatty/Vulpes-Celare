...
const AhoCorasick_1 = require("../utils/AhoCorasick");
/**
...
class HospitalDictionary {
    ...
    static init(options = {}) {
        ...
        try {
            if (fs.existsSync(hospitalsPath)) {
                ...
                this.ahoCorasick = (0, AhoCorasick_1.createAhoCorasick)(this.hospitalPhrases);
                RadiologyLogger_1.RadiologyLogger.info("DICTIONARY", `Loaded ${this.hospitals.size} hospitals (${this.hospitalPhrases.length} phrases, Aho-Corasick built)`);
            }
            else {
    ...
    static findHospitalsInText(text) {
        ...
        for (const acMatch of acMatches) {
            ...
            const charAfter = acMatch.end < lowerText.length ? lowerText[acMatch.end] : " ";
