...
const SQLiteDictionaryMatcher_1 = require("./SQLiteDictionaryMatcher");
...
class NameDictionary {
    ...
    static sqliteMatcher = null;
    static usingSQLite = false;
    static isSQLiteEnabled() {
    ...
    static init(options = {}) {
        ...
        if (this.isSQLiteEnabled()) {
            try {
                this.sqliteMatcher = (0, SQLiteDictionaryMatcher_1.getSQLiteDictionaryMatcher)();
                if (this.sqliteMatcher.available()) {
                    this.usingSQLite = true;
                    const metadata = this.sqliteMatcher.getMetadata();
                    RadiologyLogger_1.RadiologyLogger.info("DICTIONARY", `Using SQLite dictionary backend (${metadata?.total_entries || "?"} entries, 96% less memory)`);
                ...
                RadiologyLogger_1.RadiologyLogger.warn("DICTIONARY", `SQLite backend unavailable, falling back to in-memory: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
     ...
     */
    static initPhoneticMatcherFromSQLite() {
        if (this.phoneticInitialized)
            return;
        ...
        this.phoneticInitialized = true;
        RadiologyLogger_1.RadiologyLogger.info("DICTIONARY", `Using SQLite phonetic matching (soundex-indexed)`);
    }
    /**
    ...
    static isFirstName(name) {
        ...
        const unicodeNorm = UnicodeNormalizer_1.UnicodeNormalizer.quickNormalize(lower);
        // Use SQLite backend if available
        if (this.usingSQLite && this.sqliteMatcher) {
            if (this.sqliteMatcher.hasExact(lower, "first"))
                return true;
            // Try Unicode-normalized form
            if (unicodeNorm !== lower && this.sqliteMatcher.hasExact(unicodeNorm, "first"))
                return true;
            // Try OCR normalization
            const normalized = this.normalizeOCR(unicodeNorm);
            if (normalized !== unicodeNorm &&
        ...
        if (this.firstNames.has(lower))
            return true;
        // Try Unicode-normalized form
        if (unicodeNorm !== lower && this.firstNames.has(unicodeNorm))
            return true;
        // Try OCR normalization (on Unicode-normalized text)
        const normalized = this.normalizeOCR(unicodeNorm);
        if (normalized !== unicodeNorm && this.firstNames.has(normalized))
            return true;
    ...
    static isSurname(name) {
        ...
        // Use SQLite backend if available
        if (this.usingSQLite && this.sqliteMatcher) {
            if (this.sqliteMatcher.hasExact(lower, "surname"))
                return true;
            // Try Unicode-normalized form
            if (unicodeNorm !== lower && this.sqliteMatcher.hasExact(unicodeNorm, "surname"))
                return true;
            // Try OCR normalization
            const normalized = this.normalizeOCR(unicodeNorm);
            if (normalized !== unicodeNorm &&
        ...
        if (this.surnames.has(lower))
            return true;
        // Try Unicode-normalized form
        if (unicodeNorm !== lower && this.surnames.has(unicodeNorm))
            return true;
        // Try OCR normalization (on Unicode-normalized text)
        const normalized = this.normalizeOCR(unicodeNorm);
        if (normalized !== unicodeNorm && this.surnames.has(normalized))
            return true;
        // Try deduplication
        const deduplicated = this.deduplicate(normalized);
    ...
    static getNameLists() {
        ...
        // When using SQLite backend, query names from the database
        if (this.usingSQLite && this.sqliteMatcher) {
            try {
                const lists = this.sqliteMatcher.getAllNames();
                if (lists.firstNames.length > 0 || lists.surnames.length > 0) {
                    this.cachedNameLists = lists;
                    RadiologyLogger_1.RadiologyLogger.info("DICTIONARY", `Loaded ${lists.firstNames.length} first names and ${lists.surnames.length} surnames for Rust scanner`);
                    return this.cachedNameLists;
                }
            }
            catch (error) {
                RadiologyLogger_1.RadiologyLogger.warn("DICTIONARY", `Failed to load names from SQLite for Rust scanner: ${error instanceof Error ? error.message : String(error)}`);
