...
exports.fdaExporter = exports.FDAExporter = void 0;
...
|---------|---------|---------|----------|----------|
${report.sbom.map(s => `| ${s.name} | ${s.version} | ${s.license} | ${s.supplier} | ${s.isCritical ? 'Yes' : 'No'} |`).join('\n')}
...
|----|----------|-------------|-------|---------------|
${report.riskAssessment.map(r => `| ${r.id} | ${r.category} | ${r.description} | ${r.riskScore} | ${r.residualRisk} |`).join('\n')}

### Risk Mitigations

${report.riskAssessment.map(r => `**${r.id}:** ${r.mitigation}`).join('\n\n')}
...
exports.FDAExporter = FDAExporter;
// Singleton export
exports.fdaExporter = FDAExporter.getInstance();
//# sourceMappingURL=FDAExporter.js.map