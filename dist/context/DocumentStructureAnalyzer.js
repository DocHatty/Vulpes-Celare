...
class DocumentStructureAnalyzer {
     ...
     */
    static getPositionContext(text, offset, profile) {
        ...
        let confidence = 0.7;
        if (fieldContext === 'LABELED' && distance < 20)
            confidence = 0.95;
        else if (fieldContext === 'TABLE_CELL' && columnHeader)
            confidence = 0.9;
        else if (profile.type === 'FORM' && fieldContext === 'STRUCTURED')
            confidence = 0.85;
        return {
     ...
     */
    static getContextBoost(position, phiType) {
        let boost = 0;
    ...
    static findNearestLabel(text, offset) {
    ...
    static findColumnHeader(text, offset) {
        ...
        while (searchPos > 0) {
            ...
            if (lineStart - prevLineStart > 500)
                break;
