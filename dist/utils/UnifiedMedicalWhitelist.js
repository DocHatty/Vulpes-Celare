...
exports.UnifiedMedicalWhitelist = void 0;
exports.shouldWhitelist = shouldWhitelist;
exports.getWhitelistPenalty = getWhitelistPenalty;
exports.isMedicalTerm = isMedicalTerm;
exports.isNonPHI = isNonPHI;
 ...
 */
const EPONYM_DISEASE_PATTERN = /\b(disease|syndrome|sign|test|phenomenon|reflex|maneuver|law|rule|ring|node|body|cell|duct|canal|nerve|artery|vein|muscle|ligament|tendon|bone|criteria|score|scale|classification|staging|grade)\b/i;
 ...
 */
const PERSON_TITLE_PATTERN = /^(?:Dr\.?|Mr\.?|Mrs\.?|Ms\.?|Miss|Prof\.?|Rev\.?)\s+/i;
...
class UnifiedMedicalWhitelist {
    ...
    static isMedicalEponym(text, context) {
        ...
        if (context) {
            ...
            if (EPONYM_DISEASE_PATTERN.test(fullContext))
                return true;
            if (EPONYM_POSSESSIVE_PATTERN.test(fullContext))
                return true;
        }
        // Check the text itself for disease indicators
        if (EPONYM_DISEASE_PATTERN.test(text))
            return true;
        if (EPONYM_POSSESSIVE_PATTERN.test(text))
            return true;
     ...
     */
    static isMedicalCondition(text) {
        const normalized = text.toLowerCase().trim();
        if (MEDICAL_CONDITIONS.has(normalized))
            return true;
     ...
     */
    static isProcedure(text) {
        const normalized = text.toLowerCase().trim();
        if (PROCEDURES.has(normalized))
            return true;
        const words = normalized.split(/\s+/);
    ...
    static isDocumentStructure(text) {
        const normalized = text.toLowerCase().trim();
        if (DOCUMENT_STRUCTURE.has(normalized))
            return true;
    ...
    static isFieldLabel(text) {
        const normalized = text.toLowerCase().trim();
        if (FIELD_LABELS.has(normalized))
            return true;
     ...
     */
    static isInsuranceCompany(text) {
        const normalized = text.toLowerCase().trim();
    ...
    static isGeographicTerm(text) {
        const normalized = text.toLowerCase().trim();
        if (GEOGRAPHIC_TERMS.has(normalized))
            return true;
        const words = normalized.split(/\s+/);
     ...
     */
    static isPatternMatchedType(filterType) {
     ...
     */
    static shouldExcludeFromRedaction(text) {
    ...
    static isMedicalTerm(text) {
        return (this.isMedicalCondition(text) ||
     ...
     */
    static shouldWhitelist(text, filterType, context) {
        ...
        if (this.isFieldLabel(text))
            return true;
        if (this.isInsuranceCompany(text))
            return true;
        if (this.isHospitalName(text))
            return true;
        if (this.isDocumentStructure(text))
            return true;
        if (this.isMedicalTerm(text))
            return true;
        if (this.isMedicalEponym(text, context))
            return true;
        // For NAME type, also check geographic terms and never-name words
        if (filterType === Span_1.FilterType.NAME || filterType === "NAME") {
            if (this.isGeographicTerm(text))
                return true;
            if (this.isNeverNameWord(text))
                return true;
        }
        return false;
    }
     ...
     */
    static getWhitelistPenalty(text, context) {
        if (this.hasPersonIndicators(text))
            return 0;
        ...
        if (this.isMedicalEponym(text, context))
            return 0.9;
        if (this.isMedication(text))
            return 0.85;
        if (this.isMedicalCondition(text))
            return 0.85;
        if (this.isProcedure(text))
            return 0.8;
        if (this.isAnatomicalTerm(text))
            return 0.75;
        if (this.isClinicalAcronym(text))
            return 0.8;
        // Organizations
        if (this.isInsuranceCompany(text))
            return 0.9;
     ...
     */
    static addCustomTerms(category, terms) {
    ...
    static getCategorySet(category) {
        switch (category) {
            case "eponyms":
     ...
     */
    static getAllTerms() {
        if (this.cachedAllTerms)
            return this.cachedAllTerms;
...
exports.UnifiedMedicalWhitelist = UnifiedMedicalWhitelist;
// ============================================================================
// CONVENIENCE EXPORTS
// ============================================================================
/**
 ...
 */
function shouldWhitelist(text, filterType, context) {
    return UnifiedMedicalWhitelist.shouldWhitelist(text, filterType, context);
}
/**
 * Get whitelist penalty for confidence scoring
 */
function getWhitelistPenalty(text, context) {
 ...
 */
function isNonPHI(text) {
    return UnifiedMedicalWhitelist.isNonPHI(text);
}
//# sourceMappingURL=UnifiedMedicalWhitelist.js.map