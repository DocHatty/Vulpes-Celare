...
class ValidationUtils {
     ...
     */
    static isTestCardNumber(cardNumber) {
     ...
     */
    static isValidPhoneLength(phone, minDigits = 7, maxDigits = 15) {
        const digits = this.extractDigits(phone);
     ...
     */
    static isValidVanityPhone(phone) {
     ...
     */
    static isValidId(id, minLength, maxLength, requireDigit = true) {
     ...
     */
    static isValidHealthPlanId(memberId) {
    ...
    static isValidIPv6(ip) {
        ...
        if (!ip.includes(":"))
            return false;
        if (ip.length < 3 || ip.length > 45)
            return false;
        // Must not be a time format (e.g., "12:34")
        if (/^\d{1,2}:\d{2}$/.test(ip))
            return false;
     ...
     */
    static isValidGPSCoordinate(coords) {
     ...
     */
    static isValidLastFirstFormat(name) {
     ...
     */
    static isValidLastFirstFormatStrict(name) {
        const parts = name.split(",");
        if (parts.length !== 2)
            return false;
        const lastName = parts[0].trim();
        ...
        return (/^[A-Z][a-z]{2,}/.test(lastName) && /^[A-Z][a-z]{2,}/.test(firstName));
    }
    /**
     ...
     * @returns true if valid name format
     */
    static isValidFirstLastFormat(name, strict = false) {
        ...
        if (strict) {
            ...
            return (/^[A-Z][a-z]{1,}$/.test(firstName) && /^[A-Z][a-z]{1,}$/.test(lastName));
        }
        else {
            // Lenient: just needs to be alphabetic with min length
            return (/^[A-Za-z]{2,}$/.test(firstName) && /^[A-Za-z]{2,}$/.test(lastName));
        }
    }
    /**
     ...
     */
    static isValidNameComponent(word, minLength = 2) {
