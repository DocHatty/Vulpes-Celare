...
exports.NameDetectionUtils = exports.FAMILY_RELATIONSHIP_KEYWORDS = exports.NON_PERSON_STRUCTURE_TERMS = exports.PROVIDER_CREDENTIALS = exports.PROVIDER_TITLE_PREFIXES = void 0;
 ...
 */
exports.PROVIDER_CREDENTIALS = new Set([
    // Medical doctors
    "MD",
    "M.D.",
    "DO",
 ...
 */
exports.NON_PERSON_STRUCTURE_TERMS = [
    "protected health",
    "social security",
    "medical record",
    "health plan",
 ...
 */
exports.FAMILY_RELATIONSHIP_KEYWORDS = [
    "Spouse",
    "Wife",
    "Husband",
    "Father",
...
class NameDetectionUtils {
     ...
     * Matches: "SMITH, JOHN", "O'BRIEN, MARY JANE"
     */
    static getLastFirstAllCapsPattern() {
     ...
     */
    static getFamilyRelationshipPattern() {
        const keywords = exports.FAMILY_RELATIONSHIP_KEYWORDS.join("|");
        return new RegExp(`\\b(?:${keywords})[ \\t:]+([A-Z][a-z]+(?:[ \\t]+[A-Z][a-z]+){0,2})\\b`, "gi");
    }
    /**
     * Get compiled regex for age/gender descriptor names
     * Matches: "45 year old woman Jane Smith"
     */
    static getAgeGenderPattern() {
     ...
     */
    static getPossessiveNamePattern() {
     ...
     */
    static getInitialLastNamePattern() {
     ...
     */
    static getTitledNamePattern() {
        const titles = Array.from(exports.PROVIDER_TITLE_PREFIXES)
            .map((t) => t.replace(".", "\\."))
            .join("|");
        return new RegExp(`\\b(?:${titles})\\.?[ \\t]+([A-Z][a-zA-Z'-]+(?:[ \\t]+[A-Z][a-zA-Z'-]+){0,2})\\b`, "gi");
    }
    // =========================================================================
    // VALIDATION METHODS
    // =========================================================================
    ...
    static validateLastFirst(name) {
        ...
        return (/^[A-Za-z][a-zA-Z'-]{2,}$/.test(lastName) &&
     ...
     */
    static validateLastFirstStrict(name) {
        const parts = name.split(",");
        if (parts.length !== 2)
            return false;
        const lastName = parts[0].trim();
        ...
        return (/^[A-Z][a-z]{2,}$/.test(lastName) && /^[A-Z][a-z]{1,}/.test(firstName));
    }
    /**
     ...
     */
    static isNonPersonStructureTerm(text) {
        const normalized = text.toLowerCase().trim();
        // Check exact match
        if (exports.NON_PERSON_STRUCTURE_TERMS.includes(normalized)) {
            return true;
        }
        // Check if text contains any structure term
        for (const term of exports.NON_PERSON_STRUCTURE_TERMS) {
     ...
     */
    static extractContext(text, offset, length, contextSize = 150) {
     ...
     */
    static startsWithTitle(text) {
        const words = text.trim().split(/\s+/);
        if (words.length === 0)
            return false;
        const firstWord = words[0].replace(/\.$/, "");
        return (exports.PROVIDER_TITLE_PREFIXES.has(firstWord) ||
            exports.PROVIDER_TITLE_PREFIXES.has(firstWord + "."));
    }
    /**
    ...
    static removeTitle(text) {
        ...
        if (words.length <= 1)
            return trimmed;
        const firstWord = words[0].replace(/\.$/, "");
        if (exports.PROVIDER_TITLE_PREFIXES.has(firstWord) ||
            exports.PROVIDER_TITLE_PREFIXES.has(firstWord + ".")) {
     ...
     */
    static endsWithCredentials(text) {
        const words = text.trim().split(/[\s,]+/);
        if (words.length === 0)
            return false;
        ...
        for (let i = Math.max(0, words.length - 3); i < words.length; i++) {
            ...
            if (exports.PROVIDER_CREDENTIALS.has(word)) {
                return true;
            }
        }
        return false;
    ...
    static isLikelyPersonName(text, _context) {
        ...
        const firstConf = NameDictionary_1.NameDictionary.getNameConfidence(firstWord);
        const lastConf = NameDictionary_1.NameDictionary.getNameConfidence(lastWord);
        // High confidence if both are known names
        if (firstConf > 0.7 && lastConf > 0.7)
            return true;
        // Medium confidence if one is known
        if (firstConf > 0.5 || lastConf > 0.5)
            return true;
        // Check for title prefix
        if (this.startsWithTitle(trimmed))
            return true;
        return false;
    }
    /**
    ...
    static performWhitelistCheck(text, context, isAllCapsMode = false) {
        ...
        if (context && HospitalDictionary_1.HospitalDictionary.isPartOfHospitalName(text, context)) {
            return true;
        }
        ...
        for (const word of words) {
            if ((0, UnifiedMedicalWhitelist_1.isMedicalTerm)(word)) {
                return true;
            }
        }
     ...
     */
    static getFamilyRelationshipKeywords() {
        return [...exports.FAMILY_RELATIONSHIP_KEYWORDS];
    }
    /**
     * Get non-person structure terms as array
     */
    static getStructureTerms() {
        return [...exports.NON_PERSON_STRUCTURE_TERMS];
    }
    /**
     * Check if a word is a provider title
     */
    static isProviderTitle(word) {
        const normalized = word.replace(/\.$/, "");
        return (exports.PROVIDER_TITLE_PREFIXES.has(normalized) ||
            exports.PROVIDER_TITLE_PREFIXES.has(normalized + "."));
    }
    /**
     * Check if a word is a professional credential
     */
    static isCredential(word) {
        const normalized = word.replace(/[.,]/g, "");
        return exports.PROVIDER_CREDENTIALS.has(normalized);
    }
}
exports.NameDetectionUtils = NameDetectionUtils;
//# sourceMappingURL=NameDetectionUtils.js.map