...
class CLI {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // DIAGNOSE - Pipeline state and diagnostics
    // ══════════════════════════════════════════════════════════════════════════
    static async diagnose(options) {
        const { PipelineAudit } = await Promise.resolve().then(() => __importStar(require("../diagnostics/PipelineAudit")));
        if (options.json) {
            const audit = PipelineAudit.run();
            VulpesOutput_1.out.print(JSON.stringify(audit, null, 2));
            return;
        }
        // Print the full audit summary
        PipelineAudit.printSummary();
        if (options.verbose) {
            const state = PipelineAudit.getAudit();
            VulpesOutput_1.out.print("\n[FILE LOCATIONS]\n");
            VulpesOutput_1.out.print("Filters:");
            Object.entries(state.fileLocations.filters).forEach(([type, files]) => {
                VulpesOutput_1.out.print(`  ${type}:`);
                files.forEach((f) => VulpesOutput_1.out.print(`    ${f}`));
            });
            VulpesOutput_1.out.print("\nCore:");
            state.fileLocations.core.forEach((f) => VulpesOutput_1.out.print(`  ${f}`));
            VulpesOutput_1.out.print("\nDictionaries:");
            state.fileLocations.dictionaries.forEach((f) => VulpesOutput_1.out.print(`  ${f}`));
            VulpesOutput_1.out.print("\n[PIPELINE STAGES]\n");
            state.pipelineStages.forEach((stage, i) => {
                VulpesOutput_1.out.print(`  ${i + 1}. ${stage}`);
            });
        }
    }
}
