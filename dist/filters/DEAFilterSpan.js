...
class DEAFilterSpan extends SpanBasedFilter_1.SpanBasedFilter {
    ...
    getPriority() {
        return SpanBasedFilter_1.FilterPriority.MRN; // Same priority as other healthcare identifiers
    }
    detect(text, config, context) {
        const accelerated = RustScanKernel_1.RustScanKernel.getDetections(context, text, "DEA");
        if (accelerated) {
        ...
        for (const pattern of DEAFilterSpan.COMPILED_PATTERNS) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                const deaNumber = match.length > 3 && match[1] && match[2] && match[3]
