    ...
    detect(text, _config, context) {
        ...
        if (accelerated && accelerated.length > 0) {
            return accelerated.map((d) => {
                return SpanFactory_1.SpanFactory.fromPosition(text, d.characterStart, d.characterEnd, Span_1.FilterType.LICENSE, {
        ...
        for (let i = 0; i < LicenseNumberFilterSpan.COMPILED_PATTERNS.length; i++) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (value && this.validate(value)) {
                    ...
                    const span = SpanFactory_1.SpanFactory.fromPosition(text, valueStart, valueEnd, Span_1.FilterType.LICENSE, {
                        confidence: 0.88,
                        priority: this.getPriority(),
