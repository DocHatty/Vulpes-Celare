    ...
    detectPartialAddresses(text, spans, seen) {
        for (const patternDef of PARTIAL_ADDRESS_PATTERNS) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (DocumentVocabulary_1.DocumentVocabulary.isMedicalTerm(matchText) ||
                    DocumentVocabulary_1.DocumentVocabulary.isNonPHI(matchText)) {
                    continue;
                }
    ...
    detectCityNames(text, spans, seen) {
        for (const patternDef of CITY_CONTEXT_PATTERNS) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                seen.add(key);
                // Skip non-PHI terms
                if (DocumentVocabulary_1.DocumentVocabulary.isMedicalTerm(cityName) ||
                    DocumentVocabulary_1.DocumentVocabulary.isNonPHI(cityName)) {
                    continue;
                }
