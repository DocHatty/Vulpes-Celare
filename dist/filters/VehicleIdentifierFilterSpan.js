...
class VehicleIdentifierFilterSpan extends SpanBasedFilter_1.SpanBasedFilter {
     ...
     * Pattern 1: VIN with context (explicit label)
     */
    detectLabeledVINs(text, spans) {
        const pattern = /\b(?:VIN|Vehicle\s+Identification\s+Number|Vehicle\s+ID)[\s:#]*([A-HJ-NPR-Z0-9]{17})\b/gi;
        pattern.lastIndex = 0;
        let match;
     ...
     */
    detectStandaloneVINs(text, spans) {
        ...
        while ((match = pattern.exec(text)) !== null) {
            const vin = match[1];
            if (this.isValidVIN(vin)) {
                const span = this.createSpanFromMatch(text, match, Span_1.FilterType.VEHICLE, 0.85);
     ...
     * Pattern 3: License Plate with explicit context
     */
    detectLabeledLicensePlates(text, spans) {
        const pattern = /\b(?:license\s+plate|plate\s+number|registration|plate)[\s:#]*([A-Z]{2}[-\s]?[A-Z0-9]{5,7}|[A-Z0-9]{2,3}[-\s]?[A-Z0-9]{3,4})\b/gi;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const plate = match[1];
            if (this.isValidLicensePlate(plate)) {
                ...
                const span = new Span_1.Span({
                    ...
                    confidence: 0.95,
                    priority: this.getPriority(),
                    context: this.extractContext(text, plateStart, plateStart + plate.length),
                    window: [],
                    replacement: null,
                    salt: null,
     ...
     * Pattern 4: Standalone license plates with common US formats
     */
    detectStandaloneLicensePlates(text, spans) {
        ...
        for (const pattern of patterns) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (this.isValidLicensePlate(plate)) {
                    const span = this.createSpanFromMatch(text, match, Span_1.FilterType.VEHICLE, 0.75);
     ...
     */
    isVitalSignContext(text, matchIndex, matchedText) {
        // Common vital sign abbreviations
        const vitalSignPrefixes = /\b(BP|HR|RR|PR|Temp|SpO2|O2|Sat|SBP|DBP|MAP)\s*$/i;
        ...
        const surroundingText = text.substring(Math.max(0, matchIndex - 50), Math.min(text.length, matchIndex + matchedText.length + 20));
     ...
     */
    detectGPSCoordinates(text, spans) {
        const pattern = /\b(-?\d{1,3}\.\d{4,10})\s*°?\s*[NS]?,?\s*(-?\d{1,3}\.\d{4,10})\s*°?\s*[EW]?\b/gi;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const coords = match[0];
            if (this.isValidGPSCoordinate(coords)) {
                const span = this.createSpanFromMatch(text, match, Span_1.FilterType.VEHICLE, 0.92);
     ...
     */
    detectIPv6Addresses(text, spans) {
        ...
        while ((match = fullPattern.exec(text)) !== null) {
            ...
            if (this.isValidIPv6(ipv6)) {
                const span = this.createSpanFromMatch(text, match, Span_1.FilterType.VEHICLE, 0.9);
                span.pattern = "IPv6 address";
                spans.push(span);
            }
        }
        // Pattern 6b: Compressed IPv6
        const compressedPattern = /\b((?:[0-9a-fA-F]{1,4}:){1,7}:|(?:[0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|(?:[0-9a-fA-F]{1,4}:){1,5}(?::[0-9a-fA-F]{1,4}){1,2}|(?:[0-9a-fA-F]{1,4}:){1,4}(?::[0-9a-fA-F]{1,4}){1,3}|(?:[0-9a-fA-F]{1,4}:){1,3}(?::[0-9a-fA-F]{1,4}){1,4}|(?:[0-9a-fA-F]{1,4}:){1,2}(?::[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:(?:(?::[0-9a-fA-F]{1,4}){1,6})|:(?:(?::[0-9a-fA-F]{1,4}){1,7}|:))\b/g;
        compressedPattern.lastIndex = 0;
        while ((match = compressedPattern.exec(text)) !== null) {
            const ipv6 = match[1];
            if (this.isValidIPv6(ipv6)) {
                const span = this.createSpanFromMatch(text, match, Span_1.FilterType.VEHICLE, 0.88);
     ...
     * Pattern 7: Workstation IDs (explicit format)
     */
    detectExplicitWorkstationIDs(text, spans) {
        const pattern = /\b([A-Z]{2,8}-[A-Z]{2,8}-(?:STATION|TERMINAL|WS|WORKSTATION|COMPUTER)-[A-Z0-9]{1,4})\b/gi;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const span = this.createSpanFromMatch(text, match, Span_1.FilterType.VEHICLE, 0.93);
     ...
     * Pattern 8: Workstation IDs with context
     */
    detectContextualWorkstationIDs(text, spans) {
        const pattern = /\b(?:Workstation|Terminal|Station|Computer)\s+(?:ID|Number|#)[\s:#]*([A-Z0-9]{2,}-[A-Z0-9]{2,}-[A-Z0-9]{2,})\b/gi;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const fullMatch = match[0];
            ...
            const span = new Span_1.Span({
                ...
                confidence: 0.95,
                priority: this.getPriority(),
                context: this.extractContext(text, idStart, idStart + workstationID.length),
                window: [],
                replacement: null,
                salt: null,
     ...
     */
    detectVehicleMakeModelYear(text, spans) {
        const makesPattern = VehicleIdentifierFilterSpan.VEHICLE_MAKES.join("|");
        // Pattern A: Year + Make + Model (e.g., "2019 Toyota Camry")
        const patternA = new RegExp(`\\b((?:19|20)\\d{2}|'\\d{2})\\s+(${makesPattern})\\s+([A-Z][A-Z0-9\\-]{1,15}(?:\\s+[A-Z0-9\\-]{1,10})?)\\b`, "gi");
        ...
        const patternB = new RegExp(`\\b(${makesPattern})\\s+([A-Z][A-Z0-9\\-]{1,15}(?:\\s+[A-Z0-9\\-]{1,10})?)\\s+((?:19|20)\\d{2}|'\\d{2})\\b`, "gi");
        ...
        const patternC = new RegExp(`\\b(${makesPattern})\\s+([A-Z][A-Z0-9\\-]{2,15})\\b`, "gi");
        ...
        while ((match = patternC.exec(text)) !== null) {
            ...
            const isKnownModel = VehicleIdentifierFilterSpan.COMMON_MODELS.some((m) => m.toUpperCase() === model.toUpperCase() ||
                m.toUpperCase().replace(/[- ]/g, "") ===
                    model.toUpperCase().replace(/[- ]/g, ""));
            const hasVehicleContext = this.hasVehicleContext(text, match.index, fullMatch.length);
     ...
     */
    detectVehicleDescriptions(text, spans) {
        // Labeled vehicle patterns
        const pattern = /\b(?:patient'?s?\s+)?(?:vehicle|car|truck|suv|van|motorcycle|bike)\s*(?:is|was|:)?\s*(?:a\s+)?([A-Z0-9][A-Z0-9\s\-']{5,40})\b/gi;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const description = match[1];
            // Validate: should contain a known make or year
            const hasMake = VehicleIdentifierFilterSpan.VEHICLE_MAKES.some((make) => description.toUpperCase().includes(make));
     ...
     */
    hasVehicleContext(text, matchIndex, matchLength) {
     ...
     */
    isValidVehicleDescription(description) {
...
exports.VehicleIdentifierFilterSpan = VehicleIdentifierFilterSpan;
/**
 * Common vehicle makes for pattern matching
 */
VehicleIdentifierFilterSpan.VEHICLE_MAKES = [
    // American
    "FORD",
    "CHEVROLET",
    "CHEVY",
 ...
 */
VehicleIdentifierFilterSpan.COMMON_MODELS = [
    // Toyota
    "CAMRY",
    "COROLLA",
    "RAV4",
...
//# sourceMappingURL=VehicleIdentifierFilterSpan.js.map