...
class BiometricContextFilterSpan extends SpanBasedFilter_1.SpanBasedFilter {
     ...
     * Pattern 1: Sentence-level detection with biometric keywords
     */
    detectBiometricSentences(text, spans) {
        const keywordPattern = this.BIOMETRIC_KEYWORDS.map((keyword) => keyword.replace(/\s+/g, "\\s+")).join("|");
     ...
     */
    detectDescriptorPhrases(text, spans) {
        const pattern = /\b(?:shows|has|displays|visible|noted|observed)\s+(?:tattoo|scar|birthmark|identifying\s+mark)[^.!?]{0,50}[.!?]/gi;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
     ...
     * Pattern 3: Photograph/Image references
     */
    detectPhotographReferences(text, spans) {
        const pattern = /\b(?:photograph|photo|image|picture)\s+(?:of|shows|depicts|displaying)?\s*(?:patient|individual|subject)['']?s?\s+(?:face|features|appearance)[^.!?]{0,50}[.!?]/gi;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const phrase = match[0];
     ...
     * Pattern 4: DNA/Genetic test results
     */
    detectGeneticTests(text, spans) {
        const pattern = /\b(?:dna|genetic|genomic)\s+(?:test|testing|analysis|screening|marker|profile)[^.!?]{0,100}[.!?]/gi;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const phrase = match[0];
     ...
     */
    detectBiometricIdCodes(text, spans) {
        ...
        const labeledPattern = new RegExp(`\\b(?:iris|retinal?|dna|genetic|fingerprint|voiceprint|biometric|palm|facial?)\\s*(?:scan)?\\s*(?:id|identifier|code|number|#|no\\.?)\\s*[:\\-]?\\s*([A-Z0-9][A-Z0-9\\-]{4,})\\b`, "gi");
    ...
    isBiometricReference(text) {
        ...
        const hasBiometricKeyword = this.BIOMETRIC_KEYWORDS.some((keyword) => lowerText.includes(keyword.toLowerCase()));
...
exports.BiometricContextFilterSpan = BiometricContextFilterSpan;
//# sourceMappingURL=BiometricContextFilterSpan.js.map