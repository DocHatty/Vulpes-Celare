...
class HealthPlanNumberFilterSpan extends SpanBasedFilter_1.SpanBasedFilter {
    ...
    detect(text, _config, context) {
        const accelerated = RustScanKernel_1.RustScanKernel.getDetections(context, text, "HEALTHPLAN");
        if (accelerated && accelerated.length > 0) {
            return accelerated.map((d) => {
                return SpanFactory_1.SpanFactory.fromPosition(text, d.characterStart, d.characterEnd, Span_1.FilterType.HEALTH_PLAN, {
        ...
        for (let i = 0; i < HealthPlanNumberFilterSpan.COMPILED_PATTERNS.length; i++) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (this.validateHealthPlanNumber(value)) {
                    ...
                    const span = SpanFactory_1.SpanFactory.fromPosition(text, valueStart, valueEnd, Span_1.FilterType.HEALTH_PLAN, {
                        confidence: 0.85,
                        priority: this.getPriority(),
