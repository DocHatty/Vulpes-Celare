...
class FamilyNameFilterSpan extends SpanBasedFilter_1.SpanBasedFilter {
    ...
    detect(text, config, context) {
                ...
                if (name.trim().length >= 2) {
                    ...
                    if (namePos !== -1) {
                        const span = new Span_1.Span({
                            ...
                            confidence: 0.88,
                            priority: this.getPriority(),
                            context: this.extractContext(text, namePos, namePos + name.length),
                            window: [],
                            replacement: null,
                            salt: null,
        ...
        while ((match = multipleChildrenPattern.exec(text)) !== null) {
            ...
            const span1 = new Span_1.Span({
                ...
                confidence: 0.9,
                priority: this.getPriority(),
                context: this.extractContext(text, name1Start, name1Start + name1.length),
                window: [],
                replacement: null,
                salt: null,
            ...
            const span2 = new Span_1.Span({
                ...
                confidence: 0.9,
                priority: this.getPriority(),
                context: this.extractContext(text, name2Start, name2Start + name2.length),
                window: [],
                replacement: null,
                salt: null,
                pattern: "Child with age",
        ...
        while ((match = childNameWithAgePattern.exec(text)) !== null) {
            ...
            if (firstName && firstName.length >= 2) {
                ...
                const span = new Span_1.Span({
                    ...
                    confidence: 0.9,
                    priority: this.getPriority(),
                    context: this.extractContext(text, firstNameStart, firstNameStart + firstName.length),
                    window: [],
                    replacement: null,
                    salt: null,
                    pattern: "Child with age",
    ...
    looksLikePersonName(name) {
        ...
        if ((0, UnifiedMedicalWhitelist_1.shouldWhitelist)(name, Span_1.FilterType.NAME)) {
            return false;
        }
        // Use shared utility for non-person structure term check
        if (NameDetectionUtils_1.NameDetectionUtils.isNonPersonStructureTerm(name)) {
            return false;
        }
        // Check if first word is whitelisted (document terms, etc.)
        const words = name.split(/\s+/);
        if (words.length > 0 && (0, UnifiedMedicalWhitelist_1.shouldWhitelist)(words[0], Span_1.FilterType.NAME)) {
            return false;
        }
