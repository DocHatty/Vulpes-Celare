...
class FormattedNameFilterSpan extends SpanBasedFilter_1.SpanBasedFilter {
    ...
    detectLastFirstNames(text, spans) {
        ...
        const mixedCasePattern = /\b([A-Z][a-z]{2,},[ \t]*[A-Z][a-z]{2,}(?:[ \t]+[A-Z][a-z]{2,})?)\b/g;
    ...
    isProviderTitledName(matchedText) {
        return NameDetectionUtils_1.NameDetectionUtils.startsWithTitle(matchedText);
    }
    /**
     * Pattern 0.5: Titled names (Dr. Smith, Mr. Jones)
     *
    ...
    detectTitledNames(text, spans) {
        ...
        const pattern = new RegExp(`\\b(?:${NameFilterConstants_1.NAME_PREFIXES.join("|")})\\.?[ \\t]+[A-Z][a-z]+(?:[ \\t]+[A-Z][a-z]+)*\\b`, "g");
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
    ...
    detectPatientAllCapsNames(text, spans) {
        const pattern = /\b(?:Patient|Pt|Subject|Individual|Client|PATIENT|PT|SUBJECT|INDIVIDUAL|CLIENT)[ \t:]+([A-Z]{2,}(?:[ \t]+[A-Z]{2,}){1,2})\b/g;
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const name = match[1];
            ...
            if (words.length >= 2 &&
                ...
                !(0, NameFilterConstants_1.isExcludedAllCaps)(name.trim())) {
                const fullMatch = match[0];
                const matchPos = match.index;
                const nameStart = matchPos + fullMatch.indexOf(name);
                const nameEnd = nameStart + name.length;
    ...
    detectNamesWithSuffix(text, spans) {
        const pattern = new RegExp(`\\b([A-Z][a-z]+(?:[ \\t]+[A-Z]\\.?[ \\t]*)?[ \\t]+[A-Z][a-z]+)[ \\t]*,?[ \\t]*(?:${NameFilterConstants_1.NAME_SUFFIXES.join("|")})\\.?\\b`, "gi");
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(text)) !== null) {
            const name = match[1];
    ...
    isWhitelistedLastFirst(text) {
        ...
        if (words.length > 0) {
            const allMedical = words.every((word) => DocumentVocabulary_1.DocumentVocabulary.isMedicalTerm(word));
            if (allMedical)
                return true;
        }
    ...
    isLikelyPersonName(text) {
        ...
        if (NameFilterConstants_1.DOCUMENT_TERMS.has(firstWord))
            return false;
        if (NameFilterConstants_1.NON_NAME_ENDINGS.has(lastWord))
            return false;
        // Check geographic terms
        for (const word of words) {
            if (NameFilterConstants_1.GEOGRAPHIC_TERMS.has(word))
                return false;
        }
    ...
    isWhitelisted(text, isAllCapsLastFirst = false, context) {
        ...
        if (isAllCapsLastFirst) {
            ...
            const allWordsAreMedical = words.every((word) => DocumentVocabulary_1.DocumentVocabulary.isMedicalTerm(word));
     ...
     */
    detectNamesWithCredentials(text, spans) {
        ...
        const credentialPattern = new RegExp(`\\b([A-Z][a-z]+(?:\\s+[A-Z][a-z]+)?)\\s*,?\\s*(${credentials.join("|")})\\b`, "g");
        let match;
