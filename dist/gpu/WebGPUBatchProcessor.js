...
class WebGPUBatchProcessor {
    ...
    /**
     * Process with WebGPU - falls back to CPU parallel processing.
     * WebGPU implementation requires browser/Deno runtime with GPU access.
     */
    async processWithGPU(documents) {
        // WebGPU requires specific runtime environment (browser/Deno with GPU)
        // Node.js fallback uses CPU parallel processing which is still fast
        VulpesLogger_1.vulpesLogger.info("Using CPU parallel processing (WebGPU requires browser/Deno runtime)", {
            component: COMPONENT,
        });
