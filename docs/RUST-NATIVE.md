...
|----------|----------|---------|-------|
| `VULPES_WORKERS` | Worker thread pool | OFF | Set to `1` to enable parallel filter execution |
| `VULPES_NO_WORKERS` | Disable workers (legacy) | - | Use `VULPES_WORKERS=0` instead |
| `VULPES_TRACE` | Pipeline tracing | OFF | Set to `1` for debug tracing |
| `VULPES_DEBUG_OVERLAP` | Overlap resolution logs | OFF | Set to `1` for verbose overlap debugging |
| `VULPES_REQUIRE_NATIVE` | Require native bindings | OFF | Set to `1` to fail instead of falling back to TS |
