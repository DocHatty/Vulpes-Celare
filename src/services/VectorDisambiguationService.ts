...
/**
 * Vector representation of a span + context
 */
...
export class VectorDisambiguationService {
   ...
   */
  private selectBestSpan(ambiguousGroup: Span[]): Span {
   ...
   */
  private calculateDisambiguationScore(span: Span): number {
    ...
    const sameTypeVectors = cachedVectors.filter(
      (v) => v.filterType === span.filterType,
    );
   ...
   */
  private cacheObservation(span: Span): void {
    const key = this.makeWindowKey(span.window);
    const spanVector = this.createVector(span);
    ...
    const exists = cached.some(
      (v) =>
        v.filterType === spanVector.filterType &&
   ...
   */
  private createVector(span: Span): SpanVector {
  ...
  private hashToVector(text: string): number[] {
    const vector: number[] = new Array(this.config.vectorSize).fill(0);
   ...
   * MurmurHash3 (32-bit)
   */
  private murmur3Hash(str: string, seed: number = 0): number {
    let h = seed;
   ...
   */
  private makeWindowKey(window: string[]): string {
    let tokens = window;
