    ...
    if (this.usingSQLite && this.sqliteMatcher) {
      try {
        ...
        if (lists.firstNames.length > 0 || lists.surnames.length > 0) {
          ...
          RadiologyLogger.info(
            "DICTIONARY",
            `Loaded ${lists.firstNames.length} first names and ${lists.surnames.length} surnames for Rust scanner`,
          );
        ...
        RadiologyLogger.warn(
          "DICTIONARY",
          `Failed to load names from SQLite for Rust scanner: ${error instanceof Error ? error.message : String(error)}`,
        );
