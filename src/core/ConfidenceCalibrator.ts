...
export interface CalibrationDataPoint {
  confidence: number; // Raw confidence score
  isActualPHI: boolean; // Ground truth label
  filterType?: FilterType; // Optional type for type-specific calibration
}
...
/**
 * Isotonic regression model (piecewise linear)
 */
interface IsotonicModel {
  thresholds: number[]; // Input confidence thresholds
  values: number[]; // Output calibrated values
}
...
export interface CalibrationMetrics {
  ...
  reliability: CalibrationBin[]; // Reliability diagram data
}
...
export class ConfidenceCalibrator {
  private plattParams: PlattParameters | null = null;
  private isotonicModel: IsotonicModel | null = null;
  private betaParams: BetaParameters | null = null;
  private temperature: number = 1.0;
  private typeSpecificCalibrators: Map<FilterType, ConfidenceCalibrator> =
    new Map();

  private calibrationData: CalibrationDataPoint[] = [];
  private isFitted: boolean = false;
  private preferredMethod: "platt" | "isotonic" | "beta" | "temperature" =
    "isotonic";

  // Calibration parameters
  private static readonly NUM_BINS = 10;
  private static readonly MIN_BIN_COUNT = 5;
  private static readonly PLATT_ITERATIONS = 100;
  private static readonly PLATT_LEARNING_RATE = 0.01;

  constructor(
    preferredMethod: "platt" | "isotonic" | "beta" | "temperature" = "isotonic",
  ) {
  ...
  fit(): void {
    if (this.calibrationData.length < 20) {
      throw new Error(
        `Insufficient calibration data. Need at least 20 points, have ${this.calibrationData.length}`,
      );
    }
  ...
  private fitIsotonic(): void {
    ...
    const sorted = [...this.calibrationData].sort(
      (a, b) => a.confidence - b.confidence,
    );

    // Pool Adjacent Violators (PAV) algorithm
    const n = sorted.length;
    const values: number[] = sorted.map((p) => (p.isActualPHI ? 1 : 0));
    ...
    for (let j = 0; j < values.length; j++) {
      ...
      const midPoint =
        sorted[Math.floor((currentIdx + endIdx - 1) / 2)].confidence;
   ...
   */
  private fitTypeSpecific(): void {
    // Group data by type
    const typeGroups = new Map<FilterType, CalibrationDataPoint[]>();

    for (const point of this.calibrationData) {
      if (!point.filterType) continue;
   ...
   * Apply Platt scaling
   */
  private applyPlatt(confidence: number): number {
    if (!this.plattParams) return confidence;
   ...
   */
  private applyIsotonic(confidence: number): number {
    if (!this.isotonicModel) return confidence;
    ...
    for (let i = 0; i < thresholds.length - 1; i++) {
      if (confidence >= thresholds[i] && confidence <= thresholds[i + 1]) {
        const t =
          (confidence - thresholds[i]) / (thresholds[i + 1] - thresholds[i]);
   ...
   */
  private applyBeta(confidence: number): number {
    if (!this.betaParams) return confidence;
    const s = Math.max(0.001, Math.min(0.999, confidence));
    return (
      this.betaParams.a * Math.pow(s, this.betaParams.b) + this.betaParams.c
    );
  }
  ...
  computeMetrics(): CalibrationMetrics {
    if (!this.isFitted || this.calibrationData.length === 0) {
      throw new Error(
        "Calibrator must be fitted with data before computing metrics",
      );
    }
    ...
    for (const bin of bins) {
      if (bin.count > 0) {
        ...
        const calibrationError = Math.abs(
          bin.meanConfidence - bin.actualAccuracy,
        );
    ...
    return {
      ...
      reliability: bins.filter(
        (b) => b.count >= ConfidenceCalibrator.MIN_BIN_COUNT,
      ),
    };
  }
   ...
   */
  static fromLabeledData(
    predictions: Array<{ confidence: number; filterType?: FilterType }>,
    labels: boolean[],
    ...
    const data: CalibrationDataPoint[] = predictions.map((pred, i) => ({
