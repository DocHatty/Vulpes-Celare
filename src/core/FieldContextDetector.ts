...
export interface FieldContext {
  label: string; // The field label (e.g., "PATIENT", "FILE #")
  labelStart: number; // Character position of label start
  labelEnd: number; // Character position of label end
  expectedType: string; // Expected PHI type (NAME, MRN, DATE, etc.)
  valueStart: number; // Character position where value begins
  valueEnd: number; // Character position where value ends (approximate)
  value?: string; // Extracted value if found
  confidence: number; // Confidence in this detection
  isMultiLine: boolean; // Whether value is on a different line than label
}

/**
 * Field label definitions with expected value types
 */
...
export class FieldContextDetector {
  /**
   * Field label patterns mapped to expected value types
   */
  private static readonly FIELD_DEFINITIONS: FieldLabelDefinition[] = [
   ...
   */
  private static isFieldLabel(text: string): boolean {
   ...
   */
  static getExpectedTypeAtPosition(
   ...
   */
  static createContextMap(
    text: string,
  ): Map<number, { expectedType: string; confidence: number }> {
    const contexts = this.detect(text);
    const map = new Map<number, { expectedType: string; confidence: number }>();
      ...
      if (nameMatch) {
        ...
        if (isValid) {
          ...
          const isDuplicate = results.some(
            (r) => r.start === nameStart && r.end === nameEnd,
          );
