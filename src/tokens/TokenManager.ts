...
export class TokenManager {
  ...
  private tokensReinserted: number = 0;
  ...
  reinsert(llmResponse: string): string {
    ...
    const dateTokenMap = new Map<string, string>();
    ...
    if (dateTokenMap.size > 0) {
      const shiftedDatePattern =
        /\[(?:(\d+)\s+days\s+(earlier|later),\s+)?SHIFTED_DATE_(\d+):\s+(\d{4})\]/g;
      processedResponse = processedResponse.replace(
        shiftedDatePattern,
        (match, _days, _direction, dateNum) => {
    ...
    if (standardTokens.length > 0) {
      ...
      const exactPattern = new RegExp(
        standardTokens.map((t) => this.escapeRegex(t)).join("|"),
        "g",
      );
