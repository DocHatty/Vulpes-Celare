...
function generateFish(): string {
  const commandCompletions = COMMANDS.map(c => 
    `complete -c vulpes -n __fish_use_subcommand -a ${c.name} -d '${c.desc.replace(/'/g, "\\'")}'`
  ).join("\n");
...
export function generateCompletions(shell: string): void {
  ...
  switch (shell.toLowerCase()) {
    ...
    case "pwsh":
      ...
      break;
    default:
      out.print(Status.error(`Unknown shell: ${shell}`));
      out.print("\nSupported shells: bash, zsh, fish, powershell");
      out.print("\nUsage:");
      out.print("  vulpes completions bash    # Output bash completions");
      out.print("  vulpes completions zsh     # Output zsh completions");
      out.print("  vulpes completions fish    # Output fish completions");
      out.print("  vulpes completions powershell  # Output PowerShell completions");
      out.print("\nInstallation examples:");
      out.print("  vulpes completions bash >> ~/.bashrc");
      out.print("  vulpes completions zsh > ~/.zfunc/_vulpes");
      out.print("  vulpes completions fish > ~/.config/fish/completions/vulpes.fish");
      out.print('  vulpes completions powershell >> $PROFILE');
      return;
  }
...
export function showCompletionHelp(): void {
  out.print(Box.info([
    theme.bold("Shell Completions"),
    "",
    "Enable tab completion for Vulpes commands.",
    "",
    theme.secondary("Bash:"),
    ...
    "",
    theme.secondary("Fish:"),
    "  vulpes completions fish > ~/.config/fish/completions/vulpes.fish",
    "",
    theme.secondary("PowerShell:"),
    "  vulpes completions powershell >> $PROFILE",
