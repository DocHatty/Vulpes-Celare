...
import { Status, Divider, Progress, Box, Banner } from "../theme/output";
...
export class CLI {
  ...
  static async diagnose(options: { json?: boolean; verbose?: boolean }): Promise<void> {
    const { PipelineAudit } = await import("../diagnostics/PipelineAudit");
    
    if (options.json) {
      const audit = PipelineAudit.run();
      out.print(JSON.stringify(audit, null, 2));
      return;
    }
    ...
    if (options.verbose) {
      ...
      Object.entries(state.fileLocations.filters).forEach(([type, files]) => {
        out.print(`  ${type}:`);
        files.forEach((f: string) => out.print(`    ${f}`));
      });
      
      out.print("\nCore:");
      state.fileLocations.core.forEach((f: string) => out.print(`  ${f}`));
      
      out.print("\nDictionaries:");
      state.fileLocations.dictionaries.forEach((f: string) => out.print(`  ${f}`));
      
      out.print("\n[PIPELINE STAGES]\n");
      state.pipelineStages.forEach((stage: string, i: number) => {
        out.print(`  ${i + 1}. ${stage}`);
      });
    }
  }
