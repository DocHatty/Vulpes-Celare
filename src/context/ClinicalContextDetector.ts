 ...
 */
export class ClinicalContextDetector {
  private static readonly CONTEXT_WINDOW_SIZE = 150; // chars before/after
  ...
  static analyzeContext(
    ...
    const windowEnd = Math.min(
      text.length,
      position + length + this.CONTEXT_WINDOW_SIZE
    );
   ...
   */
  static getContextConfidenceBoost(
    text: string,
    position: number,
    length: number = 0
  ): number {
...
export const RELATIVE_DATE_PATTERNS: {
  ...
  {
    pattern:
      /\b(?:a|one|two|three|four|five|six)\s+months?\s+ago\b/gi,
    baseConfidence: 0.8,
    requiresContext: true,
  },
  {
  ...
  {
    pattern:
      /\b(?:last|this|next)\s+(?:spring|summer|fall|autumn|winter)\b/gi,
    baseConfidence: 0.6,
    requiresContext: true,
  },
  {
  ...
  {
    pattern:
      /\b(?:last|this|next)\s+(?:Christmas|Thanksgiving|Easter|New\s+Year'?s?)\b/gi,
    baseConfidence: 0.7,
    requiresContext: true,
  },
 ...
 */
export const contextDetector = ClinicalContextDetector;
