 ...
 */
const FILTER_TYPE_NAMES: Record<string, string> = {
  NAME: "Person Name",
...
export class ExplanationGenerator {
  ...
  static explain(
    span: Span,
    threshold: number = ExplanationGenerator.DEFAULT_THRESHOLD
   ...
   */
  static exportAuditDocument(
    result: RedactionResult,
    spans: Span[],
    options: { includeContext?: boolean; includeFactors?: boolean } = {}
  ): string {
    ...
    for (let i = 0; i < report.explanations.length; i++) {
      ...
      if (includeFactors && exp.confidenceFactors.length > 0) {
        ...
        for (const factor of exp.confidenceFactors) {
          ...
          lines.push(
            `  - ${factor.factor}: ${sign}${(factor.impact * 100).toFixed(0)}% (${factor.reason})`
          );
        }
      }
   ...
   */
  private static inferMatchSource(span: Span): string {
   ...
   */
  private static extractContextIndicators(span: Span): string[] {
    ...
    const contextPatterns: Array<{ pattern: RegExp; description: string }> = [
   ...
   */
  private static estimateBaseConfidence(span: Span): number {
