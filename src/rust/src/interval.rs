...
/// Check if text contains any structure words
fn contains_structure_word(text: &str) -> bool {
    let upper = text.to_uppercase();
    for word in upper.split_whitespace() {
        if NAME_STRUCTURE_WORDS.contains(&word) {
            return true;
        }
    }
    false
...
fn calculate_span_score(span: &SpanInput) -> f64 {
    ...
    let mut length_score = (length / 50.0).min(1.0) * 40.0;
    ...
    if span.filter_type == "NAME" && contains_structure_word(&span.text) {
        length_score = 0.0;
    }
