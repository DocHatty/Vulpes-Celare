 ...
 */
export const OcrCharacterSubstitutions: Record<string, string[]> = {
    // Numeric confusions
    "0": ["O", "o", "Q", "D"],
    "1": ["l", "I", "|", "i", "!"],
    "2": ["Z", "z"],
 ...
 */
export function generateOcrTolerantPattern(
    text: string,
    options: {
    ...
    while (i < text.length) {
        ...
        if (includeMultiChar) {
            ...
            for (const [key, subs] of Object.entries(OcrCharacterSubstitutions)) {
        ...
        const char = text[i];
        const subs = OcrCharacterSubstitutions[char];
 ...
 */
export const OcrAddressSuffixPatterns = [
...
export function normalizeOcrText(text: string): string {
    ...
    for (const [canonical, corrupted] of Object.entries(OcrCharacterSubstitutions)) {
 ...
 */
export function isOcrEquivalent(a: string, b: string): boolean {
    ...
    for (let i = 0; i < a.length; i++) {
        ...
        const subsA = OcrCharacterSubstitutions[charA] || [];
        const subsB = OcrCharacterSubstitutions[charB] || [];
...
export const OcrPatterns = {
    CharacterSubstitutions: OcrCharacterSubstitutions,
    AddressSuffixes: OcrAddressSuffixPatterns,
    StateAbbreviations: OcrStatePatterns,
    // Utility functions
