 ...
 */
export const InvalidNamePrefixes = [
 ...
 */
export const InvalidNameSuffixes = [
 ...
 */
export const MedicalPhrases = new Set([
...
export function isSectionHeading(text: string): boolean {
    ...
    if (SectionHeadings.has(trimmed)) return true;

    // Check single-word headings
    const words = trimmed.split(/\s+/);
    if (words.length === 1 && SingleWordHeadings.has(words[0])) return true;
 ...
 */
export function containsStructureWord(text: string): boolean {
    const upper = text.toUpperCase();
    const words = upper.split(/\s+/);
    return words.some((word) => StructureWords.has(word));
}

/**
 * Check if a name starts with an invalid prefix.
 */
export function hasInvalidPrefix(name: string): boolean {
    return InvalidNamePrefixes.some((prefix) => name.startsWith(prefix));
}

/**
 * Check if a name ends with an invalid suffix.
 */
export function hasInvalidSuffix(name: string): boolean {
    const lower = name.toLowerCase();
    return InvalidNameSuffixes.some((suffix) => lower.endsWith(suffix));
}

/**
 * Check if text matches a medical phrase.
 */
export function isMedicalPhrase(text: string): boolean {
    return MedicalPhrases.has(text.toLowerCase());
}
