 ...
 */
const ADDRESS_CONTEXT_PATTERNS: RegExp[] = [
...
export class ContextAwareAddressFilter extends SpanBasedFilter {
  getType(): string {
    return "ADDRESS";
  }
  ...
  private detectPartialAddresses(
    ...
    for (const patternDef of PARTIAL_ADDRESS_PATTERNS) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        if (
          DocumentVocabulary.isMedicalTerm(matchText) ||
          DocumentVocabulary.isNonPHI(matchText)
        ) {
          continue;
        }
  ...
  private detectCityNames(
    ...
    for (const patternDef of CITY_CONTEXT_PATTERNS) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        // Skip non-PHI terms
        if (
          DocumentVocabulary.isMedicalTerm(cityName) ||
          DocumentVocabulary.isNonPHI(cityName)
        ) {
          continue;
        }
   ...
   */
  private hasAddressContext(text: string, position: number): boolean {
   ...
   */
  private isStateName(name: string): boolean {
