...
export class SmartNameFilterSpan extends SpanBasedFilter {
  ...
  detect(text: string, config: any, context: RedactionContext): Span[] {
    ...
    if (rustAvailable) {
      ...
      for (const d of lastFirstDets) {
        ...
        // Match exclusions
        if (
          TITLE_PLUS_TRAILING_WORD_PATTERN.test(
            text.substring(Math.max(0, start - 30), start),
          )
        )
          continue;
        ...
        if (parts.length === 2) {
          ...
          if (PROVIDER_CREDENTIALS.has(secondPart.toUpperCase())) continue;
        }
      ...
      for (const d of firstLastDets) {
        ...
        if (
          TITLE_TRAILING_PATTERN.test(
            text.substring(Math.max(0, start - 30), start),
          )
        )
          continue;
        if (
          PROVIDER_CONTEXT_CREDENTIAL_AFTER_NAME_PATTERN.test(
            text.substring(end, Math.min(text.length, end + 40)),
          )
        )
          continue;
      ...
      RadiologyLogger.warn(
        "SMART_NAME",
        "Rust scanner unavailable - relying on simplified fallback.",
      );
    }
  ...
  private isInProviderContext(
    ...
    if (TITLE_PREFIX_PATTERN.test(beforeText)) {
      return true;
    }
    ...
    if (TITLED_NAME_LOOKBACK_PATTERN.test(beforeText)) {
      return true;
    }
    ...
    if (nameEnd < fullContext.length) {
      ...
      if (PROVIDER_CONTEXT_CREDENTIAL_AFTER_NAME_PATTERN.test(afterName)) {
        return true;
      }
    }
  ...
  private detectOcrLastFirstNames(
    text: string,
    spans: Span[],
    options: { skipCommaPatterns?: boolean } = {},
  ): void {
    const patterns = getOcrLastFirstPatternDefs();
  ...
  private detectChaosLastFirstNames(text: string, spans: Span[]): void {
    ...
    const chaosPatterns = getChaosLastFirstPatternDefs();
  ...
  private isValidChaosPart(normalized: string): boolean {
    ...
    if (shouldWhitelist(normalized, FilterType.NAME)) return false;
   ...
   */
  private normalizeOcrCharsAlt(text: string): string {
    return text
      .replace(/0/g, "o")
      .replace(/1/g, "l")
      .replace(/\|/g, "l")
  ...
  private isLikelyOcrName(
    ...
    for (const word of words) {
      ...
      if (shouldWhitelist(cleanWord, FilterType.NAME)) {
        return false;
      }
    }
    ...
    if (shouldWhitelist(cleanPhrase, FilterType.NAME)) {
      return false;
    }

    // Original checks (keep as backup)
    const medPatterns =
      /^(MG|MCG|ML|TABLET|CAPSULE|PILL)$|^[A-Z]+IN$|^[A-Z]+OL$|^[A-Z]+AL$/;
    ...
    if (
      /\b(diagnosis|medication|procedure|condition|disease|symptom|treatment|prescribed|taking|drug)[\s:]/i.test(
        context,
      )
    ) {
      return false;
  ...
  private detectChaosAwareLabeledNames(
    ...
    const chaosAnalysis = OcrChaosDetector.analyze(text, context);
  ...
  private isObviousNonName(text: string): boolean {
    ...
    if (
      /\b(diagnosis|medication|procedure|treatment|assessment|plan|history)\b/.test(
        lower,
      )
    ) {
      return true;
    ...
    // Document structure
    if (
      /\b(section|page|form|date|time|signed|undefined|null|n\/a)\b/.test(lower)
    ) {
      return true;
    }
  ...
  private isWhitelisted(text: string, context?: string): boolean {
    ...
    if (shouldWhitelist(normalized, FilterType.NAME, context)) {
      return true;
    }
    ...
    const words = normalized.split(/[\s,]+/).filter((w) => w.length > 2);
    for (const word of words) {
      if (shouldWhitelist(word, FilterType.NAME, context)) {
        return true;
      }
    }
  ...
  private detectConcatenatedNames(text: string, spans: Span[]): void {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const part1IsName =
        NameDictionary.isFirstName(part1) || NameDictionary.isSurname(part1);
      const part2IsName =
        NameDictionary.isSurname(part2) || NameDictionary.isFirstName(part2);
   ...
   */
  private detectOcrCorruptedNames(text: string, spans: Span[]): void {
    ...
    const ocrNamePattern =
      /\b([A-Z][a-zA-Z0-9@$!|]{2,}(?:\s+[A-Za-z0-9@$!|][a-zA-Z0-9@$!|]{1,}){0,2})\b/g;
    ...
    while ((match = ocrNamePattern.exec(text)) !== null) {
      ...
      // Skip if already detected
      if (this.overlapsExisting(start, end, spans)) continue;

      // Skip if in provider context
      if (this.isInProviderContext(candidate, start, text)) continue;
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        const isFirstName1 = NameDictionary.isFirstName(norm1);
        const isFirstName2 = NameDictionary.isFirstName(norm2);
        const isSurname1 = NameDictionary.isSurname(norm1);
        const isSurname2 = NameDictionary.isSurname(norm2);
      ...
      if (this.isWhitelisted(candidate, text)) continue;
