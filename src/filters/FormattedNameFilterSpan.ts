...
import { isExcludedAllCaps } from "./constants/NameFilterConstants";
...
export class FormattedNameFilterSpan extends SpanBasedFilter {
  ...
  private isWhitelistedLastFirst(text: string): boolean {
    ...
    if (words.length > 0) {
      const allMedical = words.every((word) => isMedicalTerm(word));
  ...
  private isWhitelisted(
    ...
    if (isAllCapsLastFirst) {
      ...
      const allWordsAreMedical = words.every((word) => isMedicalTerm(word));
