  ...
  detect(text: string, _config: any, context: RedactionContext): Span[] {
    ...
    if (accelerated && accelerated.length > 0) {
      return accelerated.map((d) => {
        return SpanFactory.fromPosition(text, d.characterStart, d.characterEnd, FilterType.HEALTH_PLAN, {
    ...
    for (
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        if (this.validateHealthPlanNumber(value)) {
          ...
          const span = SpanFactory.fromPosition(text, valueStart, valueEnd, FilterType.HEALTH_PLAN, {
            confidence: 0.85,
            priority: this.getPriority(),
