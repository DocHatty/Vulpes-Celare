...
export class AgeFilterSpan extends SpanBasedFilter {
  ...
  getPriority(): number {
    return FilterPriority.DATE; // Same priority as dates (age-related)
  }
  ...
  detect(text: string, _config: any, context: RedactionContext): Span[] {
    ...
    if (accelerated && accelerated.length > 0) {
      return accelerated.map((d) => {
        return SpanFactory.fromPosition(text, d.characterStart, d.characterEnd, FilterType.AGE, {
   ...
   */
  private detectExplicitAgeStatements(text: string, spans: Span[]): void {
    // Pattern for "X years old", "X year old", "X y/o", "X yo"
    const patternA =
      /\b(\d{2,3})\s*(?:years?\s+old|y\.?o\.?|yr\.?s?\s+old|years?\s+of\s+age)\b/gi;
    ...
    while ((match = patternA.exec(text)) !== null) {
      ...
      if (age >= 90) {
        ...
        const span = SpanFactory.fromPosition(text, match.index, match.index + fullMatch.length, FilterType.AGE, {
          confidence: 0.96,
          priority: this.getPriority(),
   ...
   *   "Age:                    99 years old"
   */
  private detectLabeledAges(text: string, spans: Span[]): void {
    ...
    const pattern =
      /\b(?:patient\s+)?age\s*[:\-=]\s*(\d{2,3})(?:\s*(?:years?\s*(?:old)?|y\.?o\.?|yo))?\b/gi;
   ...
   */
  private detectAgeRanges(text: string, spans: Span[]): void {
    // Pattern: X-Y years
    const patternA =
      /\b(\d{2,3})\s*[-–—to]+\s*(\d{2,3})\s*(?:years?\s+old|years?|y\.?o\.?)\b/gi;
    ...
    const patternB =
      /\b(?:between|from)\s+(\d{2,3})\s+(?:and|to)\s+(\d{2,3})\s*(?:years?\s+old|years?|y\.?o\.?)?\b/gi;
   ...
   */
  private detectOrdinalAges(text: string, spans: Span[]): void {
    // Pattern for "in his/her 90s", "in their 90s"
    const patternA =
      /\b(?:in\s+)?(?:his|her|their|the)\s+(?:early\s+|mid[- ]?|late\s+)?(90|100|110)s\b/gi;
    ...
    const patternB =
      /\b(?:early|mid[- ]?|late)\s*(90|100|110)s\s*(?:years?\s+old|y\.?o\.?|of\s+age)?\b/gi;
   ...
   */
  private detectContextualAges(text: string, spans: Span[]): void {
  ...
  private detectStandaloneAgesWithContext(text: string, spans: Span[]): void {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      if (this.hasStrongAgeContext(text, match.index, ageStr.length)) {
        const span = SpanFactory.fromPosition(text, match.index, match.index + ageStr.length, FilterType.AGE, {
  ...
  private hasStrongAgeContext(
    ...
    const textAfter = text
      .substring(
        ...
        Math.min(text.length, matchIndex + matchLength + 20),
      )
      .toLowerCase();
    ...
    if (
      strongContextPatterns.some((pattern) => pattern.test(surroundingText))
    ) {
      return true;
    }
    ...
    const parenAfter = text.substring(
      matchIndex + matchLength,
   ...
   */
  private detectDemographicAges(text: string, spans: Span[]): void {
