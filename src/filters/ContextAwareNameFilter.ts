 ...
 */
const DIVERSE_FIRST_NAME_PATTERNS: RegExp[] = [
 ...
 */
const HYPHENATED_NAME_PATTERNS: RegExp[] = [
 ...
 */
const SUFFIX_NAME_PATTERNS: RegExp[] = [
...
export class ContextAwareNameFilter extends SpanBasedFilter {
  getType(): string {
    return "NAME";
  }
   ...
   */
  private detectDiverseFirstNames(text: string, spans: Span[]): void {
    for (const pattern of DIVERSE_FIRST_NAME_PATTERNS) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        if (NameDictionary.isFirstName(name.split(/\s+/)[0])) {
          continue;
        }

        // Skip medical terms and non-PHI
        if (
          DocumentVocabulary.isMedicalTerm(name) ||
          DocumentVocabulary.isNonPHI(name)
        ) {
          continue;
        }
        ...
        const contextBoost = ClinicalContextDetector.getContextConfidenceBoost(
          text,
          start,
          name.length
        );
   ...
   */
  private detectNamesWithSuffixes(text: string, spans: Span[]): void {
   ...
   */
  private detectLabeledSingleNames(text: string, spans: Span[]): void {
    // Strong labels that definitively indicate a name follows
    const labelPattern =
      /\b(?:Patient|Pt|Client|Subject|Resident|Member)\s*(?:Name)?[:\s]+([A-Z][a-z]+)\b/gi;
