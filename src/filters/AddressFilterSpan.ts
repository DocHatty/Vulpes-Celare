...
export class AddressFilterSpan extends SpanBasedFilter {
   ...
   */
  private static readonly OCR_SUFFIX_PATTERNS: Array<{
    pattern: RegExp;
    confidence: number;
  }> = [
   ...
   */
  private static matchesOCRSuffix(word: string): {
    matches: boolean;
    confidence: number;
  } {
   ...
   */
  private detectCaseInsensitiveAddresses(text: string, spans: Span[]): void {
    const suffixPattern = AddressFilterSpan.STREET_SUFFIXES.join("|");

    // Case-insensitive street address pattern
    // Matches: "8007 market meadows", "1493 front crossing", "760 poplar pass"
    const pattern = new RegExp(
      `\\b(\\d+[A-Za-z]?)\\s+([A-Za-z][A-Za-z']+(?:\\s+[A-Za-z][A-Za-z']+)*)\\s+(${suffixPattern})(?:\\s*,?\\s*(?:apt|suite|unit|#|ste|bldg|building|floor|fl)?\\s*[A-Za-z0-9]+)?\\b`,
      "gi",
    );
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const alreadyDetected = spans.some(
        (s) =>
          (start >= s.characterStart && start < s.characterEnd) ||
      ...
      if (!/^\d+[A-Za-z]?$/.test(streetNumber)) continue;
    ...
    const ocrPattern = new RegExp(
      `\\b([lIO0-9][lIO0-9SsBb]{2,4})\\s+([A-Za-z][A-Za-z']+(?:\\s+[A-Za-z][A-Za-z']+)*)\\s+(${suffixPattern})\\b`,
      "gi",
    );
    ...
    const ocrSuffixPattern =
      /\b(\d+[A-Za-z]?)\s+([A-Za-z][A-Za-z']+(?:\s+[A-Za-z][A-Za-z']+)*)\s+([A-Za-z]{3,12})\b/gi;
