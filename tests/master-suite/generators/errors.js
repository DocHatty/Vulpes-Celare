...
const DOUBLE_LETTERS = ["ll", "tt", "nn", "ss", "mm", "rr", "pp", "ff", "cc", "ee", "oo", "dd", "gg", "bb", "zz"];
...
function applyOCRErrors(str, probability = 0.15) {
  if (Math.random() > probability) return { text: str, hasErrors: false };
...
function applyTypos(str, probability = 0.2) {
  if (Math.random() > probability || str.length < 2) return { text: str, hasErrors: false };
...
function applyTransposition(str, probability = 0.1) {
  if (Math.random() > probability || str.length < 3) return { text: str, hasErrors: false };
...
function applyDeletion(str, probability = 0.08) {
  if (Math.random() > probability || str.length < 4) return { text: str, hasErrors: false };
...
function applyErrors(value, errorLevel = "medium") {
  const probabilityMap = {
    none: { ocr: 0, typo: 0, trans: 0, double: 0, case: 0, space: 0, del: 0, ins: 0 },
    low: { ocr: 0.08, typo: 0.10, trans: 0.05, double: 0.08, case: 0.12, space: 0.05, del: 0.03, ins: 0.03 },
    medium: { ocr: 0.18, typo: 0.22, trans: 0.12, double: 0.18, case: 0.25, space: 0.10, del: 0.06, ins: 0.06 },
    high: { ocr: 0.32, typo: 0.38, trans: 0.22, double: 0.28, case: 0.40, space: 0.18, del: 0.10, ins: 0.10 },
    extreme: { ocr: 0.50, typo: 0.55, trans: 0.35, double: 0.40, case: 0.55, space: 0.25, del: 0.15, ins: 0.15 }
  };
  ...
  result = r.text; hasErrors = hasErrors || r.hasErrors;
