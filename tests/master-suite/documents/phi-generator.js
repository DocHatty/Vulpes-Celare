...
function generateCompletePHIDataset(errorLevel = "medium") {
  ...
  const email2 = Math.random() > 0.5 ? generateEmail(patientNameData.first, patientNameData.last) : null;
  ...
  const secondaryId = Math.random() > 0.7 ? generateHealthPlanID() : null;
  ...
  const diagnosis3 = random(DIAGNOSES.filter(d => d !== diagnosis1 && d !== diagnosis2));
  ...
  const medication3 = random(MEDICATIONS.filter(m => m !== medication1 && m !== medication2));
  const imagingType = random(["Chest X-Ray", "CT Scan of Abdomen/Pelvis", "MRI of Brain", "CT Angiography", "Echocardiogram"]);
  
  // ========================================
  // INSURANCE COMPANY INFO (NOT PHI)
  // ========================================
  const insuranceNames = ["Blue Cross Blue Shield", "Aetna", "United Healthcare", "Cigna", "Humana", "Kaiser Permanente", "Medicare", "Medicaid"];
  const insuranceName = random(insuranceNames);
  const secondaryInsurance = Math.random() > 0.7 ? random(insuranceNames.filter(i => i !== insuranceName)) : null;
  ...
  const dataset = {
    ...
    preferredName: Math.random() > 0.7 ? patientNameData.first : null,
    dob,
    age: ageData.age,
    ...
    zip: addressData.zip,
    unit: Math.random() > 0.7 ? `Apt ${randomInt(1, 999)}` : null,
    ...
    secondaryId,
    secondaryGroup,
    policyHolderName: Math.random() > 0.8 ? generatePatientName("first_last", errorLevel).formatted : null,
    policyHolderDob: Math.random() > 0.8 ? generateDOB(false) : null,
    insuranceCity: random(["Atlanta", "Chicago", "Dallas", "Phoenix", "Indianapolis"]),
    ...
    creditCard,
    ccExpiry,
    ccName: Math.random() > 0.5 ? patientNameData.clean : null,
    ccZip: addressData.zip,
    guarantorName: Math.random() > 0.8 ? generatePatientName("first_last", "none").formatted : null,
    guarantorPhone: Math.random() > 0.8 ? generatePhone(false) : null,
    ...
    pcpFax,
    pcpPractice: random(["Primary Care Associates", "Family Medicine Group", "Internal Medicine Clinic"]),
    ...
    referringPractice: random(["Specialty Associates", "Medical Group", "Health Partners"]),
    ...
    _groundTruthPHI: [
      // Patient names
      { type: "NAME", value: patientNameData.formatted, clean: patientNameData.clean, source: "patient" },
      { type: "NAME", value: patient2Data.formatted, clean: patient2Data.clean, source: "emergency_contact_1" },
      { type: "NAME", value: patient3Data.formatted, clean: patient3Data.clean, source: "emergency_contact_2" },
      
      // SSN
      { type: "SSN", value: ssn, source: "patient_ssn" },
      ...(guarantorSsn !== ssn ? [{ type: "SSN", value: guarantorSsn, source: "guarantor_ssn" }] : []),
      ...
      // Age 90+
      ...(ageData.needsRedaction ? [{ type: "AGE_90_PLUS", value: String(ageData.age), source: "age" }] : []),
      
      // MRN/Account
      { type: "MRN", value: mrn, source: "mrn" },
      { type: "ACCOUNT_NUMBER", value: accountNumber, source: "account_number" },
      ...
      ...(secondaryId ? [{ type: "HEALTH_PLAN_ID", value: secondaryId, source: "secondary_health_plan_id" }] : []),
    ...
    _groundTruthNonPHI: [
      ...
      { type: "PROVIDER_NAME", value: attendingNameData.formatted, source: "attending_name" },
      { type: "PROVIDER_NAME", value: pcpNameData.formatted, source: "pcp_name" },
      { type: "PROVIDER_NAME", value: surgeonNameData.formatted, source: "surgeon_name" },
      { type: "PROVIDER_NAME", value: pathologistNameData.formatted, source: "pathologist_name" },
      ...
      // Age under 90
      ...(!ageData.needsRedaction ? [{ type: "AGE_UNDER_90", value: String(ageData.age), source: "age_under_90" }] : []),
      
      // Insurance company name
      { type: "INSURANCE_COMPANY", value: insuranceName, source: "insurance_name" }
    ],
