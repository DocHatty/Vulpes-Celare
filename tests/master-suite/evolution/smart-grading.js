...
class SmartGrader {
  constructor(options = {}) {
    ...
    this.config = GRADING_PROFILES[this.profile] || GRADING_PROFILES.DEVELOPMENT;
  ...
  grade(metrics, failures = [], context = {}) {
    ...
    for (const [profileName, profileConfig] of Object.entries(GRADING_PROFILES)) {
      const grader = new SmartGrader({ profile: profileName });
      results.allProfiles[profileName] = grader.calculateScore(metrics, failures, context);
    }
  ...
  calculateScore(metrics, failures, context = {}) {
    ...
    return {
      ...
      breakdown: this.getBreakdown(baseScore, penaltyDetails, bonusDetails, finalScore)
    };
  }
  ...
  calculatePenalties(failures, context) {
    ...
    switch (config.penaltyMode) {
      case 'LINEAR':
        details.total = this.calculateLinearPenalties(details.byCriticality, config.penalties);
        break;

      case 'DIMINISHING':
        details.total = this.calculateDiminishingPenalties(details.byCriticality, config.penalties);
        break;

      case 'CAPPED':
        details.total = Math.min(
          this.calculateLinearPenalties(details.byCriticality, config.penalties),
          config.penaltyCap
        );
        break;
  ...
  calculateDiminishingPenalties(byCriticality, penalties) {
    ...
    for (const [category, data] of Object.entries(byCriticality)) {
      const basePenalty = penalties[`perMissed${this.capitalizeCategory(category)}`] || 0;
  ...
  calculateOCRWeightedPenalties(failures, config) {
    ...
    for (const failure of failures) {
      const category = this.categorizeFailure(failure);
      const basePenalty = config.penalties[`perMissed${this.capitalizeCategory(category)}`] || 0;
  ...
  calculateBonuses(metrics, failures, context) {
    ...
    if (!missedTypes.has('SSN') && bonuses.perfectSSN) {
      details.items.push({ reason: 'Perfect SSN detection', points: bonuses.perfectSSN });
      details.total += bonuses.perfectSSN;
    }

    if (!missedTypes.has('NAME') && bonuses.perfectNames) {
      details.items.push({ reason: 'Perfect name detection', points: bonuses.perfectNames });
      details.total += bonuses.perfectNames;
    }

    if (!missedTypes.has('DATE') && bonuses.perfectDates) {
      details.items.push({ reason: 'Perfect date detection', points: bonuses.perfectDates });
      details.total += bonuses.perfectDates;
    }

    // Sensitivity threshold bonuses
    if (metrics.sensitivity >= 99 && bonuses.above99Sensitivity) {
      details.items.push({ reason: 'Sensitivity above 99%', points: bonuses.above99Sensitivity });
      details.total += bonuses.above99Sensitivity;
    } else if (metrics.sensitivity >= 95 && bonuses.above95Sensitivity) {
      details.items.push({ reason: 'Sensitivity above 95%', points: bonuses.above95Sensitivity });
      details.total += bonuses.above95Sensitivity;
    }

    // Improvement bonus
    if (this.previousRun && bonuses.improvement) {
      const improvement = metrics.sensitivity - (this.previousRun.sensitivity || 0);
      if (improvement > 0.5) {
        details.items.push({
    ...
    if (this.previousRun && bonuses.noRegressions) {
      ...
      if (metrics.sensitivity >= prevSens - 0.1) {
        details.items.push({ reason: 'No sensitivity regression', points: bonuses.noRegressions });
  ...
  analyzeContext(failures, context) {
    ...
    for (const failure of failures) {
      ...
      analysis.errorLevelDistribution[level] = (analysis.errorLevelDistribution[level] || 0) + 1;
    }
    ...
    // Estimate OCR impact
    const highErrorFailures = (analysis.errorLevelDistribution['high'] || 0) +
    ...
    if (totalFailures > 0) {
      ...
      if (ocrRatio > 0.5) {
        analysis.ocrImpact.explanation = 'Majority of failures are in high-error documents. OCR tolerance is the main issue.';
        analysis.recommendations.push('Focus on OCR-tolerant pattern matching');
      } else if (ocrRatio > 0.25) {
        analysis.ocrImpact.explanation = 'Significant portion of failures are OCR-related.';
        analysis.recommendations.push('Add fuzzy matching for common OCR substitutions');
      } else {
        analysis.ocrImpact.explanation = 'Most failures occur even in clean or low-error documents.';
        analysis.recommendations.push('Review pattern matching logic for edge cases');
      }
    }

    // Type-specific recommendations
    const sortedTypes = Object.entries(analysis.typeDistribution)
      .sort((a, b) => b[1] - a[1]);

    if (sortedTypes.length > 0) {
      const [topType, count] = sortedTypes[0];
      if (count > 10) {
        analysis.recommendations.push(`Prioritize ${topType} detection (${count} failures)`);
      }
    }
  ...
  analyzeEvolution(metrics) {
    ...
    if (this.previousRun) {
      ...
      if (delta > 1) {
        ...
        analysis.insight = 'Performance is stable. Consider new approaches for further improvement.';
      } else {
  ...
  getBreakdown(baseScore, penalties, bonuses, finalScore) {
    const lines = [
      `Base Score (weighted metrics):  ${baseScore.toFixed(2)}`,
    ];

    if (penalties.total > 0) {
      lines.push(`Penalties (${penalties.mode}):      -${penalties.total.toFixed(2)}`);
...
`;
  ...
  const sortedTypes = Object.entries(r.contextAnalysis.typeDistribution)
    .sort((a, b) => b[1] - a[1]);
