/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  VULPES CORTEX - HYPOTHESIS ENGINE                                           ║
...
const HYPOTHESIS_TYPES = {
  ...
  EXTEND_PATTERN: {
    template: 'Extending {filter} pattern to handle {variant} will catch {count} more cases',
  ...
  ADD_DICTIONARY_ENTRY: {
    template: 'Adding "{entry}" to {dictionary} will catch {count} missed {phiType} cases',
    requiredFields: ['entry', 'dictionary', 'count', 'phiType'],
    category: 'DICTIONARY_ENHANCEMENT'
  },
  ENABLE_FUZZY_DICTIONARY: {
    template: 'Enabling fuzzy matching on {dictionary} will improve {phiType} detection by {percentage}%',
  ...
  ADJUST_THRESHOLD: {
    template: 'Adjusting {parameter} from {oldValue} to {newValue} will improve {metric} by {improvement}%',
    requiredFields: ['parameter', 'oldValue', 'newValue', 'metric', 'improvement'],
  ...
  // Context hypotheses
  ADD_CONTEXT_RULE: {
    template: 'Adding context rule for "{context}" will improve {phiType} precision by {improvement}%',
  ...
  ADD_OCR_SUBSTITUTION: {
    template: 'Adding OCR substitution {from}→{to} will catch {count} more cases',
...
class HypothesisEngine {
  ...
  createHypothesis(type, params, evidence = {}) {
    ...
    for (const field of typeInfo.requiredFields) {
      if (params[field] === undefined) {
        throw new Error(`Missing required field: ${field} for hypothesis type ${type}`);
      }
    }
    ...
    const hypothesis = {
      ...
      predictions: {
        ...
        expectedChange: params.improvement || params.estimate || params.reduction || 0,
    ...
    switch (category) {
      case 'OCR_CONFUSION':
        ...
        for (const { from, to, count } of ocrChars) {
          hypotheses.push(this.createHypothesis('ADD_OCR_SUBSTITUTION', {
            from, to, count
          }, {
      ...
      case 'DICTIONARY_MISS':
        ...
        for (const phiType of uniquePhiTypes) {
          ...
          hypotheses.push(this.createHypothesis('ADD_DICTIONARY_ENTRY', {
            entry: uniqueEntries.join(', '),
            dictionary: this.getDictionaryForType(phiType),
            count,
            phiType
          }, {
            patterns: patterns.filter(p => p.phiType === phiType).slice(0, 5),
      ...
      case 'FORMAT_VARIATION':
        ...
        for (const phiType of uniquePhiTypes) {
          ...
          const formats = typePatterns.flatMap(p => p.details?.detected_formats || []);
          const uniqueFormats = [...new Set(formats)];

          hypotheses.push(this.createHypothesis('EXTEND_PATTERN', {
      ...
      case 'CASE_VARIATION':
        ...
        for (const phiType of uniquePhiTypes) {
          const count = patterns.filter(p => p.phiType === phiType).length;
          hypotheses.push(this.createHypothesis('ENABLE_FEATURE', {
      ...
      case 'CONTEXT_DEPENDENT':
        ...
        for (const phiType of uniquePhiTypes) {
          const typePatterns = patterns.filter(p => p.phiType === phiType);
          hypotheses.push(this.createHypothesis('ADD_CONTEXT_RULE', {
      ...
      default:
        ...
        hypotheses.push(this.createHypothesis('GENERAL_IMPROVEMENT', {
  ...
  recordResults(hypothesisId, results) {
    ...
    const total = this.data.stats.totalValidated + this.data.stats.totalInvalidated;
    if (total > 0) {
    ...
    if (this.kb && validation.status !== 'TESTING') {
      const relation = validation.status === 'VALIDATED' ? 'validated' : 'invalidated';
  ...
  validateHypothesis(hypothesis) {
    ...
    // Handle direction
    const improved = hypothesis.predictions.direction === 'INCREASE'
      ? actual > 0
      : actual < 0;
