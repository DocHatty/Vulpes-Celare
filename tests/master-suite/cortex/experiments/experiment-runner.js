...
class ExperimentRunner extends EventEmitter {
  ...
  createExperiment(config) {
    const experiment = {
      id: `EXP-${Date.now()}-${random().toString(36).substr(2, 9)}`,
      ...
      config: {
        ...
        confidenceThreshold:
          config.confidenceThreshold || EXPERIMENT_CONFIG.significanceThreshold,
  ...
  async runBaseline(experiment, testRunner) {
    ...
    experiment.baseline.aggregatedMetrics = this.aggregateMetrics(
      experiment.baseline.runs,
    );
  ...
  async runTreatment(experiment, testRunner) {
    ...
    experiment.treatment.aggregatedMetrics = this.aggregateMetrics(
      experiment.treatment.runs,
    );
  ...
  async analyzeResults(experiment) {
    ...
    if (analysis.improvements.length > 0 && analysis.regressions.length === 0) {
      ...
      // Mixed - need to weigh
      const improvementScore = analysis.improvements.reduce(
        (sum, i) => sum + Math.abs(i.percentChange),
        0,
      );
      const regressionScore = analysis.regressions.reduce(
        (sum, r) => sum + Math.abs(r.percentChange),
        0,
      );
  ...
  isStatisticallySignificant(baselineStats, treatmentStats, n) {
    ...
    const pooledStdDev = Math.sqrt(
      (Math.pow(baselineStats.stdDev, 2) + Math.pow(treatmentStats.stdDev, 2)) /
        2,
    );
  ...
  async concludeExperiment(experiment) {
    ...
    if (
      ...
      if (
        experiment.config.rollbackOnRegression &&
        ...
        conclusion.recommendation =
          "Treatment automatically rolled back due to regression";
      }
    } else if (analysis.overallEffect === "MIXED") {
      conclusion.accepted = false;
      conclusion.reason =
        "Treatment showed mixed results - improvements and regressions";
      conclusion.recommendation =
        "REVIEW manually and decide based on priorities";
    } else {
      conclusion.accepted = false;
      conclusion.reason = "No significant change detected";
      conclusion.recommendation =
        "Treatment had no measurable effect - consider different approach";
    }
  ...
  getRecentExperiments(limit = 10) {
    return this.data.experiments.slice(-limit).reverse();
  }

  getSuccessfulExperiments() {
    return this.data.experiments.filter(
      (e) => e.status === EXPERIMENT_STATUS.COMPLETE && e.conclusion?.accepted,
    );
  }
