/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  VULPES CORTEX - CODEBASE ANALYZER                                           ║
...
const CODEBASE_PATHS = {
  ...
  dictionaries: path.join(__dirname, '..', '..', '..', '..', 'src', 'dictionaries'),
...
class CodebaseAnalyzer {
  ...
  saveStates() {
    ...
    fs.writeFileSync(this.stateStoragePath, JSON.stringify(this.states, null, 2));
  }
  ...
  analyzeFilters() {
    ...
    const filterFiles = fs.readdirSync(this.paths.filters)
      .filter(f => f.endsWith('.ts') && f.includes('Filter'));

    for (const file of filterFiles) {
      const filterInfo = this.analyzeFilterFile(path.join(this.paths.filters, file));
      if (filterInfo) {
    ...
    const hipaaTypes = [
      'NAME', 'SSN', 'DATE', 'PHONE', 'FAX', 'EMAIL', 'ADDRESS', 'ZIPCODE',
      'MRN', 'HEALTH_PLAN_ID', 'ACCOUNT_NUMBER', 'LICENSE', 'VEHICLE',
      'DEVICE', 'URL', 'IP', 'BIOMETRIC', 'PHOTO'
    ];
  ...
  analyzeFilterFile(filePath) {
    try {
      ...
      if (content.includes('fuzzy') || content.includes('levenshtein') || content.includes('similarity')) {
  ...
  extractPHIType(content, fileName) {
    ...
    // Infer from filename
    const nameMap = {
      'SSN': 'SSN', 'Phone': 'PHONE', 'Email': 'EMAIL', 'Date': 'DATE',
      'Name': 'NAME', 'Address': 'ADDRESS', 'MRN': 'MRN', 'NPI': 'NPI',
      'DEA': 'DEA', 'CreditCard': 'CREDIT_CARD', 'IP': 'IP', 'URL': 'URL',
      'Fax': 'FAX', 'ZipCode': 'ZIPCODE', 'Account': 'ACCOUNT_NUMBER',
      'HealthPlan': 'HEALTH_PLAN_ID', 'License': 'LICENSE', 'Vehicle': 'VEHICLE',
      'Age': 'AGE', 'Biometric': 'BIOMETRIC', 'Device': 'DEVICE',
      'Passport': 'PASSPORT', 'UniqueIdentifier': 'UNIQUE_ID'
    };
  ...
  extractPatterns(content) {
    ...
    // Find regex patterns
    const regexMatches = content.matchAll(/(?:new RegExp\(|\/)[^\/\n]+(?:\/[gimsu]*|\))/g);
    for (const match of regexMatches) {
      if (match[0].length < 200) {  // Skip very long patterns
        patterns.push({
          type: 'regex',
  ...
  assessComplexity(content) {
    ...
    else if (lines > 100 || conditionals > 8 || loops > 5) complexity = 'MEDIUM';
  ...
  analyzePipeline() {
    ...
    if (fs.existsSync(this.paths.core)) {
      const coreFiles = fs.readdirSync(this.paths.core)
    ...
    if (fs.existsSync(this.paths.services)) {
      const serviceFiles = fs.readdirSync(this.paths.services)
  ...
  analyzeMainFile(filePath) {
    try {
      ...
      return {
        ...
        hasParallelProcessing: content.includes('parallel') || content.includes('Promise.all'),
        hasLogging: content.includes('Logger') || content.includes('console.log')
      };
    } catch (e) {
  ...
  computeFileHashes() {
    ...
    const dirsToHash = [this.paths.filters, this.paths.dictionaries, this.paths.core, this.paths.services];

    for (const dir of dirsToHash) {
      if (!fs.existsSync(dir)) continue;

      const files = fs.readdirSync(dir).filter(f => f.endsWith('.ts') || f.endsWith('.txt'));
      for (const file of files) {
  ...
  compareSnapshots(before, after) {
    ...
    const allFiles = new Set([...Object.keys(beforeHashes), ...Object.keys(afterHashes)]);
    ...
    const beforeFilters = new Set(before.filters?.filters?.map(f => f.name) || []);
    const afterFilters = new Set(after.filters?.filters?.map(f => f.name) || []);
  ...
  suggestImprovementAreas(state) {
    ...
    if (!allCapabilities.includes('FUZZY_MATCHING')) {
      suggestions.push('Consider adding fuzzy matching to improve OCR tolerance');
    }
