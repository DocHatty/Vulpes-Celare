...
describe("VulpesCelare.redactWithProvenance", () => {
  it("returns redaction result with embedded Trust Bundle", async () => {
    const result = await VulpesCelare.redactWithProvenance(sampleClinicalNote);

    // Standard redaction result fields
    ...
    // Trust Bundle embedded
    expect(result.trustBundle).toBeTruthy();
    expect(result.trustBundle.manifest).toBeTruthy();
    expect(result.trustBundle.certificate).toBeTruthy();
    expect(result.trustBundle.redactedDocument).toBe(result.text);
  });

  it("accepts custom provenance options", async () => {
    const result = await VulpesCelare.redactWithProvenance(sampleClinicalNote, {
    ...
    expect(result.trustBundle.manifest.documentId).toBe("doc-12345");
    expect(result.trustBundle.certificate.issuer.organization).toBe("Test Clinic");
  });

  it("auto-exports when exportPath provided", async () => {
    const dir = makeTempDir("vulpes-auto-export");
    try {
      const outputPath = path.join(dir, "auto-export.red");
      const result = await VulpesCelare.redactWithProvenance(sampleClinicalNote, {
      ...
      const verification = await VulpesCelare.verifyTrustBundle(outputPath);
      expect(verification.valid).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("full workflow: redact -> export -> verify", async () => {
    const dir = makeTempDir("vulpes-full-workflow");
    try {
      // Step 1: Redact with provenance
      const result = await VulpesCelare.redactWithProvenance(
        sampleClinicalNote,
        {
          policyName: "maximum",
          organizationName: "Workflow Test Hospital",
          documentId: "workflow-doc-001",
        },
      );

      // Step 2: Export
      const bundlePath = path.join(dir, "workflow-bundle.red");
      await VulpesCelare.exportTrustBundle(result.trustBundle, bundlePath);
      expect(fs.existsSync(bundlePath)).toBe(true);

      // Step 3: Verify
      const verification = await VulpesCelare.verifyTrustBundle(bundlePath);
      expect(verification.valid).toBe(true);
      expect(verification.errors).toHaveLength(0);
      expect(verification.manifest?.documentId).toBe("workflow-doc-001");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
