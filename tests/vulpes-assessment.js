...
// ============================================================================
// VULPES CELARE ASSESSMENT ENGINE
// ============================================================================
async function runAssessment() {
  console.log("=".repeat(80));
  console.log("VULPES CELARE - COMPREHENSIVE PHI REDACTION ASSESSMENT");
  console.log("=".repeat(80));
  console.log("");
  ...
  try {
    ...
    VulpesCelare = require("../dist/VulpesCelare.js").VulpesCelare;
    console.log(VulpesCelare.NAME + " v" + VulpesCelare.VERSION + " loaded");
  } catch (err) {
  ...
  for (const doc of documents) {
    const redactedText = await VulpesCelare.redact(doc.content);
    ...
    for (const phi of doc.expectedPHI) {
      ...
      if (!redactedText.includes(phi.value)) {
        ...
        docMissed.push(phi.type + ": \"" + phi.value + "\"");
      }
    }
    ...
    for (const nonPhi of doc.expectedNonPHI) {
      ...
      if (redactedText.includes(nonPhi.value)) {
        ...
        docFalsePos.push(nonPhi.type + ": \"" + nonPhi.value + "\"");
      }
    }
    ...
    if (docMissed.length > 0) {
      failures.push({ docId: doc.id, type: doc.type, missed: docMissed });
    }
    if (docFalsePos.length > 0) {
      falsePositives.push({ docId: doc.id, type: doc.type, falsePositives: docFalsePos });
    }
  }

  // Calculate metrics
  const sensitivity = totalExpectedPHI > 0 ? (totalRedactedPHI / totalExpectedPHI) * 100 : 100;
  ...
  const grade = score >= 98 ? "A+" : score >= 95 ? "A" : score >= 90 ? "B" : score >= 80 ? "C" : "F";
  ...
  if (failures.length > 0) {
    ...
    failures.forEach(function(f) {
      f.missed.forEach(function(m) {
        const type = m.split(":")[0];
        byType[type] = (byType[type] || 0) + 1;
      });
    });
    Object.entries(byType).sort((a, b) => b[1] - a[1]).forEach(function([type, count]) {
    ...
    failures.slice(0, 10).forEach(function(f) {
      ...
      f.missed.forEach(function(m) { console.log("    - " + m); });
    });
    console.log("");
  }
  ...
  if (falsePositives.length > 0) {
    ...
    console.log("-".repeat(40));
    falsePositives.slice(0, 5).forEach(function(f) {
      console.log("  Doc " + f.docId + " (" + f.type + "):");
      f.falsePositives.forEach(function(fp) { console.log("    - " + fp); });
    });
    console.log("");
  }

  // Save results
  const resultsPath = path.join(__dirname, "results", "vulpes-assessment.json");
  fs.mkdirSync(path.dirname(resultsPath), { recursive: true });
  fs.writeFileSync(resultsPath, JSON.stringify({
    timestamp: new Date().toISOString(),
    engine: VulpesCelare.NAME,
    ...
    score: score,
    grade: grade,
    totalExpectedPHI: totalExpectedPHI,
    totalRedactedPHI: totalRedactedPHI,
    totalExpectedNonPHI: totalExpectedNonPHI,
    totalPreservedNonPHI: totalPreservedNonPHI,
...
// Run
runAssessment().catch(function(err) {
