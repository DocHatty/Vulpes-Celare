...
export class StatisticalTests {
  ...
  private readonly useContinuityCorrection: boolean;
   ...
   */
  private mcnemarsExact(b: number, c: number): TestResult {
    const n = b + c;
    if (n === 0) {
      return {
  ...
  permutationTest(
    ...
    const pooledStd = Math.sqrt(
      (this.variance(samplesA) + this.variance(samplesB)) / 2
    );
  ...
  buildMcNemarTable(
    ...
    if (
      predictionsA.length !== predictionsB.length ||
    ...
    for (let i = 0; i < groundTruth.length; i++) {
      ...
      else if (aCorrect && !bCorrect) aCorrectBIncorrect++;
      else if (!aCorrect && bCorrect) aIncorrectBCorrect++;
      else bothIncorrect++;
    }
  ...
  private interpretEffectSize(
    value: number,
    type: 'cohens_d' | 'correlation' | 'log_odds'
  ): 'negligible' | 'small' | 'medium' | 'large' {
    const thresholds: Record<typeof type, [number, number, number]> = {
 ...
 */
export function createStatisticalTests(options?: {
  alpha?: number;
  useContinuityCorrection?: boolean;
}): StatisticalTests {
